...
namespace Shougun.Core.BusinessManagement.MitsumoriNyuryoku
{
    ...
    public partial class MitsumoriNyuryokuForm : SuperForm
    {
        ...
        //quoc-begin
        private Shougun.Core.BusinessManagement.MitsumoriNyuryoku.InitialPopupFormLogic logicPopup;
        //quoc-end
        /// <summary>
        /// 見積入力のSYSTE_ID
        /// </summary>
        ...
        //quoc-begin
        public string MOD_STATUS = string.Empty;
        public string MOD_SHOUNIN_NICHIJI1 = string.Empty;
        //quoc-end
        ...
        public string BIKOU1 = string.Empty;
        //quoc-end
        #region コンストラクタ
        /// <summary>
        /// コンストラクタ
            ...
            : base(WINDOW_ID.T_MITSUMORI_NYUURYOKU, windowType)
        {
            ...
            try
            {
                ...
                if (formParem.cyohyoType == 0)
                {
                    ...
                    //Đặt "1: Đơn giá báo giá (ngang)" làm loại báo giá mặc định
                    //quoc-sheet4.4-begin
                    formParem.cyohyoType = MitumorisyoConst.MITUMOTISYO_TANKA_TATE;
                    //quoc-sheet4.4-end
                }
                ...
                //quoc-begin
                if (!string.IsNullOrEmpty(formParem.MOD_STATUS))
                {
                    this.MOD_STATUS = formParem.MOD_STATUS;
                }
                if (!string.IsNullOrEmpty(formParem.MOD_SHOUNIN_NICHIJI))
                {
                    this.MOD_SHOUNIN_NICHIJI1 = formParem.MOD_SHOUNIN_NICHIJI;
                }
                //quoc-end
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(formParem.mitsumoriNumber) && WindowType_Back.Equals(WINDOW_TYPE.NEW_WINDOW_FLAG))
                {
                    ...
                    //取引先、業者、現場ロストフォーカスイベント(Khách hàng, nhà cung cấp, sự kiện mất tiêu điểm trang web)
                ...
                this.pnl_uriage.Location = new System.Drawing.Point(this.lb_MistumoriKoumoku4.Location.X, this.lb_MistumoriKoumoku4.Location.Y);
                this.pnl_new.Location = new System.Drawing.Point(this.pnl_uriage.Location.X, this.pnl_uriage.Location.Y + 70);
                this.MOD_GOUKEI_KINGAKU_INJI.ReadOnly = true;
                this.MOD_RDO_ZEIKOMI.Enabled = false;
                this.MOD_RDO_ZEINUKI.Enabled = false;
        ...
        public virtual void ChangeNewWindow(object sender, EventArgs e)
        {
            ...
            try
            {
                // 新規権限がある場合処理続行(Tiếp tục xử lý nếu có thẩm quyền mới)
                //
        ...
        public virtual void ShowPreview(object sender, EventArgs e)
        {
            ...
            // 見積書種類のみ必須チェック
            //Chỉ kiểm tra bắt buộc đối với loại báo giá
        ...
        public virtual void CopyData(object sender, EventArgs e)
        {
            ...
            // 新規権限がある場合処理続行
            //Tiếp tục xử lý nếu có thẩm quyền mới
            if (Manager.CheckAuthority("G276", WINDOW_TYPE.NEW_WINDOW_FLAG))
            {
                // 処理モードを新規にする
                //Chế độ xử lý mới
                ...
                if (catchErr) { return; }
                //取引先、業者、現場タブ画面表示設定
                //Đối tác kinh doanh, thương nhân, cài đặt hiển thị màn hình tab trang web
                if (!this.logic.Torihikisaki_GyoushaCD_GenbaCD_SetTab(true)) { return; }

                // 複写不要なデータをクリア
                // Xóa dữ liệu không cần sao chép
        ...
        public virtual void RegistMistumori(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // 取引先CDと拠点CDの関連チェック
                //Kiểm tra tương quan giữa CD nhà cung cấp và CD cơ sở
                if (this.txt_Torihikisaki_hikiai_flg.Text.Equals("0"))
                {
                    // 画面の取引先CDが「既存取引先」の場合
                    //Khi CD đối tác kinh doanh trên màn hình là "Tài khoản hiện có"
                ...
                switch (this.WindowType)
                {
                    ...
                    case WINDOW_TYPE.UPDATE_WINDOW_FLAG:
                        ...
                        //Đặt các mục kiểm tra bắt buộc (thay đổi linh hoạt tùy thuộc vào nút được nhấn)
                        ...
                        // 引合チェック関連チェック
                        //Yêu cầu kiểm tra kiểm tra liên quan
                        if (!base.RegistErrorFlag)
                        {
                        ...
                        bool ret = true;
                        // 空ページはページ間に残せません。
                        //Không thể để các trang trống giữa các trang
                        if (!base.RegistErrorFlag)
                        {
                ...
                if (!base.RegistErrorFlag)
                {
                    switch (this.WindowType)
                    {
                        ...
                        case WINDOW_TYPE.UPDATE_WINDOW_FLAG:
                            ...
                            // 登録後表示制御
                            //Kiểm soát hiển thị sau khi đăng ký
                            this.logic.GetAllEntityData(out catchErr);
                            if (catchErr) { return; }
        ...
        private void CustomDataGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            try
            {
                ...
                // 伝票番号を作成時にこのイベント処理しない
                //Không xử lý sự kiện này khi tạo số phiếu
                if (CreateDenpyoNumbering)
                {
                    return;
                }
                ...
                // 選択行番号取得
                //Nhận số dòng đã chọn
                int intRowNo = e.RowIndex;

                // 小計行は必須チェックしない。
                //Không kiểm tra hàng tổng phụ là bắt buộc
                var shoukeiFlg = this.CustomDataGridView.Rows[intRowNo].Cells[MitumorisyoConst.MITSUMORI_COLUMN_NAME_SHOUKEI_FLG];
                if (shoukeiFlg.Value != null && shoukeiFlg.Value.ToString() == "True")
                {
                    //((DgvCustomAlphaNumTextBoxCell)this.CustomDataGridView.Rows[intRowNo].Cells[MitumorisyoConst.MITSUMORI_COLUMN_NAME_HINMEI_CD]).FocusOutCheckMethod = null;
                ...
                switch (cellname)
                {
                    case MitumorisyoConst.MITSUMORI_COLUMN_NAME_HINMEI_CD:
                        ...
                        // 品名CDの入力が無ければ処理中断
                        //Nếu không có đầu vào CD tên sản phẩm, quá trình xử lý sẽ bị gián đoạn
                        ...
                        //Tôi muốn hiển thị cửa sổ bật lên ngay cả khi nhập cùng một mã tên sản phẩm,
                        //vì vậy hãy xóa phân loại tài liệu
                        ...
                        if (targetRow != null)
                        {
                            ...
                            if (string.IsNullOrEmpty(Convert.ToString(this.CustomDataGridView.Rows[intRowNo].Cells[MitumorisyoConst.MITSUMORI_COLUMN_NAME_DENPYOU_KBN_CD].Value)) || control.TextBoxChanged == true)
                            {
                                ...
                                //quoc-begin-eidt
                                if (this.txt_TorihikisakiCD.Text == string.Empty
                                    || this.txt_GyoushaCD.Text == string.Empty
                                    || this.txt_GenbaCD.Text == string.Empty
                                    || String.IsNullOrEmpty(Convert.ToString(targetRow.Cells[MitumorisyoConst.MITSUMORI_COLUMN_NAME_UNIT_CD].Value)))
                                {
                                    return;
                                }
                                //quoc-end
                                // 伝票区分選択ポップアップ表示
                                //Màn hình bật lên lựa chọn phân loại tài liệu
                    ...
                    case MitumorisyoConst.MITSUMORI_COLUMN_NAME_UNIT_CD:
                        {
                            this.logic.CalcTanka(this.CustomDataGridView.CurrentRow);
                        }
                        break;
                    //quoc-sheet4.3-end
                    default:
        ...
        /// Xử lý đã được xác thực (Tôi muốn sử dụng nó ngay cả sau khi cửa sổ bật lên được hiển thị, vì vậy tôi đã biến nó thành một phương thức)
        /// </summary>
        public virtual bool txt_TorihikisakiCD_AfterUpdate(out bool catchErr)
        {
            ...
            try
            {
                if (this.isInputError || this.beforTorihikisakiCD != this.txt_TorihikisakiCD.Text || this.beforTorihikisakiHikiai != this.txt_Torihikisaki_hikiai_flg.Text)
                {
                    ...
                    //Khôi phục thông tin lưới dữ liệu về dữ liệu đã chọn
        ...
        #region 取引先のValidatingイベント(Sự kiện xác thực tài khoản)
        /// <summary>
        /// 取引先のValidatedイベント
        /// </summary>
        /// <param name="sender"></param>
        ...
        /// <summary>
        /// Validated処理(ポップアップ表示後でも使いたいのでメソッド化)
        /// Xử lý đã được xác thực (Tôi muốn sử dụng nó ngay cả sau khi
        /// cửa sổ bật lên được hiển thị, vì vậy tôi đã biến nó thành một phương thức)
        /// </summary>
        ...
        {
            // 取引先拠点チェックがエラーだったらここのエラーは表示しない
            //Nếu kiểm tra cơ sở khách hàng có lỗi thì không hiển thị lỗi này
            if (!this.logic.torihikisakiAndKyotenErrorFlg)
            {
                //if (this.beforGyousaCD == this.txt_GyoushaCD.Text)
                //{
                ...
                // 入力が変更された場合
                //nếu đầu vào thay đổi
                if (!this.ManualInputFlg &&
                    this.beforGyousaCD != this.txt_GyoushaCD.Text)
                {
                    // 手入力フラグをTRUEに設定
                    //Đặt cờ nhập thủ công thành TRUE
                    this.ManualInputFlg = true;
                }
        ...
        /// <summary>
        /// Validated処理(ポップアップ表示後でも使いたいのでメソッド化)
        /// Xử lý đã được xác thực 
        /// (Tôi muốn sử dụng nó ngay cả sau khi cửa sổ bật lên được hiển thị,
        /// Vì vậy tôi đã biến nó thành một phương thức)
        /// </summary>
        ...
        private void dtp_MitsumoriDate_ValueChanged(object sender, EventArgs e)
        {
            ...
            //Nếu bạn không kiểm tra giá trị trước đó, các chi tiết sẽ được tính toán lại.
            DateTime tempDate = DateTime.Now;
        ...
        //quoc-begin
        private void GYOUSHA_URIAGE_CD_Enter(object sender, EventArgs e)
        {
            this.DENPYOU_KBN_CD.Text = "1";
        }

        private void GYOUSHA_SHIHARAI_CD_Enter(object sender, EventArgs e)
        {
            this.DENPYOU_KBN_CD.Text = "2";
        }

        private void MOD_KAKUIN_INJI_TextChanged(object sender, EventArgs e)
        {
            if (this.MOD_KAKUIN_INJI.Text == "1")
            {
                this.MOD_RDO_ARI.Checked = true;
                this.MOD_RDO_NASHI.Checked = false;
            }

            else if (this.MOD_KAKUIN_INJI.Text == "2")
            {
                this.MOD_RDO_ARI.Checked = false;
                this.MOD_RDO_NASHI.Checked = true;
            }
            else
            {
                this.MOD_RDO_ARI.Checked = false;
        ...
        private void MOD_GOUKEI_KINGAKU_INJI_TextChanged(object sender, EventArgs e)
        {
            if (this.MOD_GOUKEI_KINGAKU_INJI.Text == "1")
            {
                this.MOD_RDO_ZEIKOMI.Checked = true;
                this.MOD_RDO_ZEINUKI.Checked = false;
            }

            else if (this.MOD_KAKUIN_INJI.Text == "2")
            {
                this.MOD_RDO_ZEIKOMI.Checked = false;
                this.MOD_RDO_ZEINUKI.Checked = true;
            }
            else
            {
                this.MOD_RDO_ZEIKOMI.Checked = false;
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            {
                var textbox = ControlUtility.GetActiveControl(this) as TextBox;
                if (textbox.Name.ToString() == "txt_Bikou1" && textbox.Multiline)
                {
                    e.Handled = true;
        ...
        private string pre_bikou = string.Empty;
        private string value = string.Empty;
        private void txt_Bikou1_Validating(object sender, CancelEventArgs e)
        {   //quoc-begin
            if (this.Checked_txt_Bikou1()) 
            {
                return; 
            }
            this.RenderBikou();
        ...
        public void RenderBikou()
        {
            // Render text string 
            this.pre_bikou = string.Empty;
            if (!string.IsNullOrEmpty(this.txt_Bikou1.Text))
            {
                var newLinesRegex = new Regex(@"\r\n", RegexOptions.Singleline);
                var lines = newLinesRegex.Split(this.txt_Bikou1.Text);
                value = string.Empty;
                if (lines != null && lines.Count() > 0)
                {
                    for (int i = 0; i < lines.Count(); i++)
                    {
                        ...
                        int count = countLength1byte(lines[i]);
                        if (count < 78)
                        {
                            value += lines[i] + Environment.NewLine;
                        }
                        else
                        {
                            string schar_1 = lines[i].SubStringByByte(76, 8);
                            for (int k = 0; k < newLinesRegex.Split(schar_1).Count(); k++)
                            {
                                if (k.Equals(newLinesRegex.Split(schar_1).Count() - 1))
                                {
                                    value += newLinesRegex.Split(schar_1)[k] + Environment.NewLine;
                                }
                                else
                                {
                                    value += newLinesRegex.Split(schar_1)[k] + "\r\n";
                                }
                            }
                        }
                        if (!i.Equals(lines.Count() - 1))
                        {
                            pre_bikou += lines[i] + Environment.NewLine;
                        }
                        else
                        {
                            pre_bikou += lines[i];
                ...
                this.txt_Bikou1.Text = value;
            }
        }
        private bool Checked_txt_Bikou1()
        {
            bool err = false;
            if (!string.IsNullOrEmpty(this.txt_Bikou1.Text))
            {
                if (this.txt_Bikou1.Text.Split('\r').Count() > 8)
                {
                    this.errmessage.MessageBoxShowError("MAX行は８行までとする。");
                    this.txt_Bikou1.Text = this.value;
                    err = true;
                }
            }
            return err;
        }
        #region Hàm đếm ký tự của string
        private const string Endcoding = "Shift_JIS";
        private static char[] SpecialCharTwoByte = new char[] { '㎡', '㎥' };
        private int countLength1byte(string prmContent)
        {
            int count = 0;
            if (!String.IsNullOrEmpty(prmContent))
            {
                ...
                {
                    count += this.GetByteCountByIndex(encoding, arrayChar, i);
                }
            }
            return count;
        }
        private int GetByteCountByIndex(Encoding encoding, char[] arrayChar, int index)
        {
            int result = 0;
        ...
        {
            popupForm = new InitialPopupForm();

            //Popup メニューを表示
            popupForm.ShowDialog();

            this.MOD_BIKOU1.Text = this.fixStringNull(popupForm.value1);
            this.MOD_BIKOU2.Text = this.fixStringNull(popupForm.value2);
            this.MOD_BIKOU3.Text = this.fixStringNull(popupForm.value3);
            this.MOD_BIKOU4.Text = this.fixStringNull(popupForm.value4);
            this.MOD_BIKOU5.Text = this.fixStringNull(popupForm.value5);
            this.MOD_BIKOU6.Text = this.fixStringNull(popupForm.value6);
            this.MOD_BIKOU7.Text = this.fixStringNull(popupForm.value7);
            this.MOD_BIKOU8.Text = this.fixStringNull(popupForm.value8);
            this.MOD_BIKOU9.Text = this.fixStringNull(popupForm.value9);
            this.MOD_BIKOU10.Text = this.fixStringNull(popupForm.value10);
            this.MOD_BIKOU11.Text = this.fixStringNull(popupForm.value11);
            this.MOD_BIKOU12.Text = this.fixStringNull(popupForm.value12);
            this.MOD_BIKOU13.Text = this.fixStringNull(popupForm.value13);
            this.MOD_BIKOU14.Text = this.fixStringNull(popupForm.value14);
            this.MOD_BIKOU15.Text = this.fixStringNull(popupForm.value15);
            this.MOD_BIKOU16.Text = this.fixStringNull(popupForm.value16);
            this.MOD_BIKOU17.Text = this.fixStringNull(popupForm.value17);
            this.MOD_BIKOU18.Text = this.fixStringNull(popupForm.value18);
            this.MOD_BIKOU19.Text = this.fixStringNull(popupForm.value19);
            this.MOD_BIKOU20.Text = this.fixStringNull(popupForm.value20);
            this.MOD_BIKOU21.Text = this.fixStringNull(popupForm.value21);
            this.MOD_BIKOU22.Text = this.fixStringNull(popupForm.value22);
            this.MOD_BIKOU23.Text = this.fixStringNull(popupForm.value23);
            this.MOD_BIKOU24.Text = this.fixStringNull(popupForm.value24);
            this.MOD_BIKOU25.Text = this.fixStringNull(popupForm.value25);


        }
        
        public string fixStringNull(Object a)
        {
