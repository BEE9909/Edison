...
namespace Shougun.Core.BusinessManagement.MitsumoriNyuryoku
{
    ...
    public class ReportInfoR425_R508_R547_R548 : ReportInfoBase
    {
        ...
        private const int ConstMaxDispDetailVRowCount = 15;           // 金額見積もり縦のDetail1の最大表示行数
        ...
        protected override void CreateDataTableInfo()
        {
            ...
            for (int pageNo = 1; pageNo <= this.DataTablePageList.Count; pageNo++)
            {
                ...
                // 帳票出力用データの設定処理
                //Quy trình thiết lập dữ liệu cho đầu ra biểu mẫu
                                ...
                                if (!this.IsDBNull(dataTableTmp.Rows[i].ItemArray[index]))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(denpyou_kbn))
                                    {
                                        if (denpyou_kbn.Equals("支払"))
                                        {
                                            ...
                                            string sinput2 = Convert.ToString(dataTableFooterTmp.Rows[0].ItemArray[index]);
                                            if (!this.IsDBNull(dataTableFooterTmp.Rows[0].ItemArray[index]))
                                            {
                                                sinput2 = sinput2.Replace("-", "▲");
            ...
            switch (this.OutputType)
            {
                case OutputTypeDef.KingakuMitsumoriV:       // 金額見積り（縦）
                    ...
                    if (dataTableHeaderTmp.Rows.Count > 0)
                    {
                        ...
                        indexquoc = dataTableHeaderTmp.Columns.IndexOf("MOD_INEI_DATA");
                        if (!this.IsDBNull(dataTableHeaderTmp.Rows[0].ItemArray[index]))
                        {
                            ...
                            string quocstring = Convert.ToString(dataTableHeaderTmp.Rows[0].ItemArray[indexquoc]);
                            if (quocval == 1)
                            {
                                this.SetFieldVisible("MOD_PHY_KAKUIN1_VLB",true);
                                this.SetFieldVisible("Field2", true);
                                //quoc-sheet4.4-begin
                                if (!string.IsNullOrEmpty(quocstring))
                                {
                                    this.SetFieldImage("Field17", quocstring);
                                }
                                //quoc-sheet4.4-end
                            }
                            else
                    ...
                    if (dataTableHeaderTmp.Rows.Count > 0)
                    {
                        ...
                        if (!this.IsDBNull(dataTableHeaderTmp.Rows[0].ItemArray[index]))
                        {
                            ...
                            else
                            {
                                ...
                                //quoc-sheet6.1-begin
                                this.SetFieldName("PHY_MITSUMORI_NUMBER_VLB", string.Empty);
                                this.SetFieldName("PHY_MITSUMORI_NUMBER_FLB", string.Empty);
                                //quoc-sheet6.1-end
                            }
                        }
                        else
