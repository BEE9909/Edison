...
namespace Shougun.Core.BusinessManagement.MitsumoriNyuryoku
{
    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        //quoc-begin
        internal InitialPopupForm popupInit;
        //quoc-end
        /// <summary>
        /// DBアクセッサー
        /// </summary>
        ...
        /// <summary>
        /// UIFormの入力コントロール名一覧
        /// Danh sách tên điều khiển đầu vào UIForm
        /// </summary>
        /// quoc-begin
        ...
        /// ヘッダ部
        /// </summary>
        private string[] headerReportItemName = { "MITSUMORI_NUMBER", "MITSUMORI_DATE", "TORIHIKISAKI_NAME1", "TORIHIKISAKI_NAME2", "GYOUSHA_NAME1", "GYOUSHA_NAME2", "GENBA_NAME1", "GENBA_NAME2", "KENMEI", "MITSUMORI_KOUMOKU1", "MITSUMORI_1", "MITSUMORI_KOUMOKU2", "MITSUMORI_2", "MITSUMORI_KOUMOKU3", "MITSUMORI_3", "MITSUMORI_KOUMOKU4", "MITSUMORI_4", "CORP_NAME", "CORP_DAIHYOU", "KYOTEN_NAME_1", "KYOTEN_POST_1", "KYOTEN_ADDRESS1_1", "KYOTEN_ADDRESS2_1", "KYOTEN_TEL_1", "KYOTEN_FAXL_1", "KYOTEN_NAME_2", "KYOTEN_POST_2", "KYOTEN_ADDRESS1_2", "KYOTEN_ADDRESS2_2", "KYOTEN_TEL_2", "KYOTEN_FAXL_2", "BUSHO_NAME_LABEL", "BUSHO_NAME", "EIGYO_TANTOUSHA_NAME", "GOUKEI_KINGAKU", "ZEI_KBN_CD", "TORIHIKISAKI_TEL", "MOD_KAKUIN_INJI", "MOD_GOUKEI_KINGAKU_INJI", "MOD_SHAIN_TEL", "TODOUFUKEN_NAME", "TORIHIKISAKI_ADDRESS1", "TORIHIKISAKI_ADDRESS2", "TORIHIKISAKI_BUSHO_NAME", "TANTOUSHA" , "TENWABANGOU", "MAIL_ADDRESS", "MOD_BIKOU1", "MOD_BIKOU2", "MOD_BIKOU3", "MOD_BIKOU4", "MOD_BIKOU5", "MOD_BIKOU6", "MOD_BIKOU7", "MOD_BIKOU8", "MOD_BIKOU9", "MOD_BIKOU10", "MOD_BIKOU11", "MOD_BIKOU12", "MOD_BIKOU13", "MOD_BIKOU14", "MOD_BIKOU15", "MOD_BIKOU16", "MOD_BIKOU17", "MOD_BIKOU18", "MOD_BIKOU19", "MOD_BIKOU20", "MOD_BIKOU21", "MOD_BIKOU22", "MOD_BIKOU23", "MOD_BIKOU24", "MOD_BIKOU25", "MOD_INEI_DATA" };
        ...
        /// <summary>
        /// 必須チェックの設定を初期化します
        /// Khởi tạo cài đặt cho kiểm tra bắt buộc
        /// </summary>
        internal bool RequiredSettingInit()
        {
            ...
            try
            {
                ...
                //quoc-sheet1.3-begin
                this.form.MOD_KAKUIN_INJI.RegistCheckMethod = null;
                this.form.MOD_FAX_SOUFUJOU_INJI.RegistCheckMethod = null;
                //quoc-sheet1.3-end
                ...
                {
                    // CustomDataGridViewを取得する(Nhận CustomDataGridView)
        ...
        /// <summary>
        /// 必須チェックの設定を動的に生成
        /// Tự động tạo cấu hình cho các kiểm tra bắt buộc
        /// </summary>
        ...
        internal bool SetRequiredSetting()
        {
            ...
            try
            {
                ...
                //quoc-sheet1.3-begin
                this.form.MOD_KAKUIN_INJI.RegistCheckMethod = excitChecks;
                this.form.MOD_FAX_SOUFUJOU_INJI.RegistCheckMethod = excitChecks;
                //quoc-sheet1.3-end
                ...
                #region 明細必須チェック(Mục Yêu cầu Kiểm tra)
                ...
                // ページ分データ作成 明細必須チェック(Tạo dữ liệu cho trang Kiểm tra các mục bắt buộc)
                for (int page = 0; page < this.form.tbc_MitsumoriMeisai.TabPages.Count; page++)
                {
        ...
        internal void WindowsItemsSet()
        {
            ...
            if (this.IsRequireData())
            {
                ...
                //Khách hàng, nhà cung cấp, sự kiện mất tiêu điểm trang web
                bool catchErr = false;
            ...
            try
            {
                ...
                for (int page = 0; page < this.form.tbc_MitsumoriMeisai.TabPages.Count; page++)
                {
                    ...
                    for (int i = 0; i < cdgv.Rows.Count - 1; i++)
                    {
                        ...
                        // 小計行は伝票番号を入れない
                        //Các hàng tổng phụ không chứa số phiếu
        ...
        public bool WindowInit()
        {
            ...
            try
            {
                ...
                // 見積書種類ポップアップ初期表示処理
                //Quá trình hiển thị ban đầu bật lên loại báo giá
        ...
        public bool EventInit()
        {
            ...
            try
            {
                #region ボタン押下イベント(Tạo sự kiện nhấn nút trước)

                // 前へボタン押下イベント生成(Tạo sự kiện nhấn nút trước)
                this.form.btn_Mae.Click += new EventHandler(this.form.PreMitsumoriData);

                // 次へボタン押下イベント生成(Tạo sự kiện nhấn nút tiếp theo)
                this.form.btn_Tsugi.Click += new EventHandler(this.form.NextMitsumoriData);

                // 引合登録ボタンイベント生成(Tạo sự kiện nút đăng ký yêu cầu)
        ...
        #region ボタン情報の設定
        /// <summary>
        /// ボタン情報の設定(Cài đặt thông tin nút)
        /// </summary>
        ...
        public void WindowsItemsInit()
        {
            ...
            // 伝票区分一覧取得
            // Nhận danh sách phân loại phiếu
            ...
            this.form.MOD_SHOUNIN_MONO_CD.Text = string.Empty;
            this.form.MOD_SHOUNIN_MONO_NAME.Text = string.Empty;
            this.form.MOD_TXT_STATUS.Text = string.Empty;
            this.form.MOD_SHOUNIN_NICHIJI.Text = string.Empty;

            //quoc-begin
            this.form.MOD_SHIHARAI_GURUUPU_CD.Text = string.Empty;
            this.form.MOD_URIAGE_GURUUPU_CD.Text = string.Empty;
            this.form.MOD_SHIHARAI_GURUUPU_NAME.Text = string.Empty;
            this.form.MOD_URIAGE_GURUUPU_NAME.Text = string.Empty;
            this.form.MOD_KAKUIN_INJI.Text = "1";
            this.form.MOD_GOUKEI_KINGAKU_INJI.Text = "1";
            this.form.MOD_FAX_SOUFUJOU_INJI.Text = "1";
            this.form.MOD_TENWABANGOU.Text = string.Empty;
            this.form.MOD_SHOUNIN_MONO_CD.Text = string.Empty;
            this.form.txt_ShainNameRyaku.ReadOnly = true;
            this.form.MITSUMORISYURUI_CD.Text = "2";
            //quoc-end
            // 新規モードの場合
            if (this.form.WindowType.Equals(WINDOW_TYPE.NEW_WINDOW_FLAG))
            {
        ...
        /// Nhận thông tin mới nhất về tất cả các thực thể được xác định trong Logic
        /// </summary>
        ...
        public bool GetAllEntityData(out bool catchErr)
        {
            ...
            {
                // 更新前データを保持しておく
                //Bảo toàn dữ liệu trước khi cập nhật
                this.beforDto = new DTOClass();
                // 初期化
                //Khởi tạo
                this.dto = new DTOClass();
                ...
                // 画面のモードに依存しないデータの取得
                //Truy xuất dữ liệu độc lập với chế độ màn hình
                ...
                {
                    // 消費税率取得
                    //Mua lại thuế suất thuế tiêu thụ
        ...
        private void WindowsSetEntrtyData()
        {
            ...
            try
            {
                ...
                //quoc-begin
                if (this.dto.entryEntity.MOD_URIAGE_GURUUPU_CD != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_URIAGE_GURUUPU_CD.ToString()))
                {
                    this.form.MOD_URIAGE_GURUUPU_CD.Text = this.dto.entryEntity.MOD_URIAGE_GURUUPU_CD.ToString();
                }

                if (this.dto.entryEntity.MOD_TXT_STATUS != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_TXT_STATUS.ToString()))
                {
                    this.form.MOD_TXT_STATUS.Text = this.dto.entryEntity.MOD_TXT_STATUS.ToString();
                }

                if (this.dto.entryEntity.MOD_SHOUNIN_NICHIJI != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_SHOUNIN_NICHIJI.ToString()))
                {
                    this.form.MOD_SHOUNIN_NICHIJI.Text = this.dto.entryEntity.MOD_SHOUNIN_NICHIJI.ToString();
                }


                if (this.dto.entryEntity.MOD_URIAGE_GURUUPU_NAME != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_URIAGE_GURUUPU_NAME.ToString()))
                {
                    this.form.MOD_URIAGE_GURUUPU_NAME.Text = this.dto.entryEntity.MOD_URIAGE_GURUUPU_NAME.ToString();
                }

                if (this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_CD != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_CD.ToString()))
                {
                    this.form.MOD_SHIHARAI_GURUUPU_CD.Text = this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_CD.ToString();
                }

                if (this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_NAME != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_NAME.ToString()))
                {
                    this.form.MOD_SHIHARAI_GURUUPU_NAME.Text = this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_NAME.ToString();
                }
                //sheet-2
                if (!this.dto.entryEntity.MOD_KAKUIN_INJI.IsNull && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_KAKUIN_INJI.ToString()))
                {
                    this.form.MOD_KAKUIN_INJI.Text = this.dto.entryEntity.MOD_KAKUIN_INJI.ToString();
                }
                //sheet-2
                if (!this.dto.entryEntity.MOD_GOUKEI_KINGAKU_INJI.IsNull && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_GOUKEI_KINGAKU_INJI.ToString()))
                {
                    this.form.MOD_GOUKEI_KINGAKU_INJI.Text = this.dto.entryEntity.MOD_GOUKEI_KINGAKU_INJI.ToString();
                }

                if (!this.dto.entryEntity.MOD_FAX_SOUFUJOU_INJI.IsNull && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_FAX_SOUFUJOU_INJI.ToString()))
                {
                    this.form.MOD_FAX_SOUFUJOU_INJI.Text = this.dto.entryEntity.MOD_FAX_SOUFUJOU_INJI.ToString();
                }
                if (this.dto.entryEntity.MOD_TENWABANGOU != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_TENWABANGOU.ToString()))
                {
                    this.form.MOD_TENWABANGOU.Text = this.dto.entryEntity.MOD_TENWABANGOU.ToString();
                }

                if (this.dto.entryEntity.SHAIN_CD != null && !string.IsNullOrEmpty(this.dto.entryEntity.SHAIN_CD.ToString()))
                {
                    form.MOD_SHOUNIN_MONO_CD.Text = this.dto.entryEntity.SHAIN_CD.ToString();
                }
                if (this.dto.entryEntity.SHAIN_NAME != null && !string.IsNullOrEmpty(this.dto.entryEntity.SHAIN_NAME.ToString()))
                {
                    form.MOD_SHOUNIN_MONO_NAME.Text = this.dto.entryEntity.SHAIN_NAME.ToString();
                }

                if(this.dto.entryEntity.MOD_BIKOU1 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU1.ToString()))
                {
                    this.form.MOD_BIKOU1.Text = this.dto.entryEntity.MOD_BIKOU1;
                }
                if (this.dto.entryEntity.MOD_BIKOU2 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU2.ToString()))
                {
                    this.form.MOD_BIKOU2.Text = this.dto.entryEntity.MOD_BIKOU2;
                }
                if (this.dto.entryEntity.MOD_BIKOU3 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU3.ToString()))
                {
                    this.form.MOD_BIKOU3.Text = this.dto.entryEntity.MOD_BIKOU3;
                }
                if (this.dto.entryEntity.MOD_BIKOU4 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU4.ToString()))
                {
                    this.form.MOD_BIKOU4.Text = this.dto.entryEntity.MOD_BIKOU4;
                }
                if (this.dto.entryEntity.MOD_BIKOU5 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU5.ToString()))
                {
                    this.form.MOD_BIKOU5.Text = this.dto.entryEntity.MOD_BIKOU5;
                }
                if (this.dto.entryEntity.MOD_BIKOU6 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU6.ToString()))
                {
                    this.form.MOD_BIKOU6.Text = this.dto.entryEntity.MOD_BIKOU6;
                }
                if (this.dto.entryEntity.MOD_BIKOU7 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU7.ToString()))
                {
                    this.form.MOD_BIKOU7.Text = this.dto.entryEntity.MOD_BIKOU7;
                }
                if (this.dto.entryEntity.MOD_BIKOU8 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU8.ToString()))
                {
                    this.form.MOD_BIKOU8.Text = this.dto.entryEntity.MOD_BIKOU8;
                }
                if (this.dto.entryEntity.MOD_BIKOU9 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU9.ToString()))
                {
                    this.form.MOD_BIKOU9.Text = this.dto.entryEntity.MOD_BIKOU9;
                }
                if (this.dto.entryEntity.MOD_BIKOU10 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU10.ToString()))
                {
                    this.form.MOD_BIKOU10.Text = this.dto.entryEntity.MOD_BIKOU10;
                }
                if (this.dto.entryEntity.MOD_BIKOU11 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU11.ToString()))
                {
                    this.form.MOD_BIKOU11.Text = this.dto.entryEntity.MOD_BIKOU11;
                }
                if (this.dto.entryEntity.MOD_BIKOU12 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU12.ToString()))
                {
                    this.form.MOD_BIKOU12.Text = this.dto.entryEntity.MOD_BIKOU12;
                }
                if (this.dto.entryEntity.MOD_BIKOU13 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU13.ToString()))
                {
                    this.form.MOD_BIKOU13.Text = this.dto.entryEntity.MOD_BIKOU13;
                }
                if (this.dto.entryEntity.MOD_BIKOU14 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU14.ToString()))
                {
                    this.form.MOD_BIKOU14.Text = this.dto.entryEntity.MOD_BIKOU14;
                }

                if (this.dto.entryEntity.MOD_BIKOU15 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU15.ToString()))
                {
                    this.form.MOD_BIKOU15.Text = this.dto.entryEntity.MOD_BIKOU15;
                }
                if (this.dto.entryEntity.MOD_BIKOU16 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU16.ToString()))
                {
                    this.form.MOD_BIKOU16.Text = this.dto.entryEntity.MOD_BIKOU16;
                }
                if (this.dto.entryEntity.MOD_BIKOU17 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU17.ToString()))
                {
                    this.form.MOD_BIKOU17.Text = this.dto.entryEntity.MOD_BIKOU17;
                }
                if (this.dto.entryEntity.MOD_BIKOU18 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU18.ToString()))
                {
                    this.form.MOD_BIKOU18.Text = this.dto.entryEntity.MOD_BIKOU18;
                }
                if (this.dto.entryEntity.MOD_BIKOU19 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU19.ToString()))
                {
                    this.form.MOD_BIKOU19.Text = this.dto.entryEntity.MOD_BIKOU19;
                }
                if (this.dto.entryEntity.MOD_BIKOU20 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU20.ToString()))
                {
                    this.form.MOD_BIKOU20.Text = this.dto.entryEntity.MOD_BIKOU20;
                }
                if (this.dto.entryEntity.MOD_BIKOU21 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU21.ToString()))
                {
                    this.form.MOD_BIKOU21.Text = this.dto.entryEntity.MOD_BIKOU21;
                }
                if (this.dto.entryEntity.MOD_BIKOU22 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU22.ToString()))
                {
                    this.form.MOD_BIKOU22.Text = this.dto.entryEntity.MOD_BIKOU22;
                }
                if (this.dto.entryEntity.MOD_BIKOU23 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU23.ToString()))
                {
                    this.form.MOD_BIKOU23.Text = this.dto.entryEntity.MOD_BIKOU23;
                }
                if (this.dto.entryEntity.MOD_BIKOU24 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU24.ToString()))
                {
                    this.form.MOD_BIKOU24.Text = this.dto.entryEntity.MOD_BIKOU24;
                }
                if (this.dto.entryEntity.MOD_BIKOU25 != null && !string.IsNullOrEmpty(this.dto.entryEntity.MOD_BIKOU25.ToString()))
                {
                    this.form.MOD_BIKOU25.Text = this.dto.entryEntity.MOD_BIKOU25;
                }

                if (this.form.MOD_STATUS != null && !string.IsNullOrEmpty(this.form.MOD_STATUS.ToString()))
                {
                    form.MOD_TXT_STATUS.Text = this.form.MOD_STATUS.ToString();
                }

                if (this.form.MOD_SHOUNIN_NICHIJI1 != null && !string.IsNullOrEmpty(this.form.MOD_SHOUNIN_NICHIJI1.ToString()))
                {
                    form.MOD_SHOUNIN_NICHIJI.Text = this.form.MOD_SHOUNIN_NICHIJI1.ToString();
                }
        ...
        public bool CreateEntityAndUpdateTables(bool taiyuuKbn, bool errorFlag)
        {
            try
            {
                ...
                //Từ UIForm vì quá trình cập nhật DB diễn ra trong CreateEntity và mỗi quá trình cập nhật
                // 排他制御する
                //kiểm soát độc quyền
        ...
        public virtual void CreateEntity(bool tairyuuKbnFlag)
        {
            ...
            // MisumorilEntityを作成 (Tạo MisumorilEntity)
        ...
        public virtual void CreateMitsumoriEntity(bool tairyuuKbnFlag)
        {
            ...
            switch (this.form.WindowType)
            {
                ...
                case WINDOW_TYPE.UPDATE_WINDOW_FLAG:
                    ...
                    // システムID
                    // Việc đánh số bị bỏ qua vì SYSTEM_ID nhận được khi màn hình hiển thị
                    // id hệ thống
                    ...
                    this.dto.entryEntity.DELETE_FLG = false;
                    // 更新前伝票は論理削除
                    //Tài liệu trước khi cập nhật bị xóa một cách hợp lý
            ...
            //quoc-begin
            if (!string.IsNullOrEmpty(this.form.MOD_URIAGE_GURUUPU_CD.Text))
            {
                this.dto.entryEntity.MOD_URIAGE_GURUUPU_CD = this.form.MOD_URIAGE_GURUUPU_CD.Text;
            }
            else
            {
                this.dto.entryEntity.MOD_URIAGE_GURUUPU_CD = string.Empty;
            }

            if (!string.IsNullOrEmpty(this.form.MOD_SHIHARAI_GURUUPU_CD.Text))
            {
                this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_CD = this.form.MOD_SHIHARAI_GURUUPU_CD.Text;
            }
            else
            {
                this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_CD = string.Empty;
            }

            if (!string.IsNullOrEmpty(this.form.MOD_URIAGE_GURUUPU_NAME.Text))
            {
                this.dto.entryEntity.MOD_URIAGE_GURUUPU_NAME = this.form.MOD_URIAGE_GURUUPU_NAME.Text;
            }

            if (!string.IsNullOrEmpty(this.form.MOD_SHIHARAI_GURUUPU_NAME.Text))
            {
                this.dto.entryEntity.MOD_SHIHARAI_GURUUPU_NAME = this.form.MOD_SHIHARAI_GURUUPU_NAME.Text;
            }
            //quoc-sheet1.1,sheet3.2-begin
            if (!string.IsNullOrEmpty(this.form.MOD_KAKUIN_INJI.Text))
            {
                this.dto.entryEntity.MOD_KAKUIN_INJI = SqlInt16.Parse(this.form.MOD_KAKUIN_INJI.Text);
            }
            //quoc-sheet1.1,sheet3.2-end
            if (!string.IsNullOrEmpty(this.form.MOD_GOUKEI_KINGAKU_INJI.Text))
            {
                this.dto.entryEntity.MOD_GOUKEI_KINGAKU_INJI = SqlInt16.Parse(this.form.MOD_GOUKEI_KINGAKU_INJI.Text);
            }

            if (!string.IsNullOrEmpty(this.form.MOD_TENWABANGOU.Text))
            {
                this.dto.entryEntity.MOD_TENWABANGOU = this.form.MOD_TENWABANGOU.Text;
            }

            if (!string.IsNullOrEmpty(this.form.MOD_FAX_SOUFUJOU_INJI.Text))
            {
                this.dto.entryEntity.MOD_FAX_SOUFUJOU_INJI = SqlInt16.Parse(this.form.MOD_FAX_SOUFUJOU_INJI.Text);
            }

            if (!string.IsNullOrEmpty(this.form.MOD_SHOUNIN_MONO_CD.Text))
            {
                this.dto.entryEntity.MOD_SHOUNIN_SHA_CD = this.form.MOD_SHOUNIN_MONO_CD.Text;
            }

            if (!string.IsNullOrEmpty(this.form.MOD_SHOUNIN_MONO_NAME.Text))
            {
                this.dto.entryEntity.MOD_SHOUNIN_SHA_NAME = this.form.MOD_SHOUNIN_MONO_NAME.Text;
            }

            if (!string.IsNullOrEmpty(this.form.MOD_TXT_STATUS.Text))
            {
                this.dto.entryEntity.MOD_TXT_STATUS = this.form.MOD_TXT_STATUS.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_SHOUNIN_NICHIJI.Text))
            {
                this.dto.entryEntity.MOD_SHOUNIN_NICHIJI = this.form.MOD_SHOUNIN_NICHIJI.Text;
            }


            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU1.Text))
            {
                this.dto.entryEntity.MOD_BIKOU1 = this.form.MOD_BIKOU1.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU2.Text))
            {
                this.dto.entryEntity.MOD_BIKOU2 = this.form.MOD_BIKOU2.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU3.Text))
            {
                this.dto.entryEntity.MOD_BIKOU3 = this.form.MOD_BIKOU3.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU4.Text))
            {
                this.dto.entryEntity.MOD_BIKOU4 = this.form.MOD_BIKOU4.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU5.Text))
            {
                this.dto.entryEntity.MOD_BIKOU5 = this.form.MOD_BIKOU5.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU6.Text))
            {
                this.dto.entryEntity.MOD_BIKOU6 = this.form.MOD_BIKOU6.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU7.Text))
            {
                this.dto.entryEntity.MOD_BIKOU7 = this.form.MOD_BIKOU7.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU8.Text))
            {
                this.dto.entryEntity.MOD_BIKOU8 = this.form.MOD_BIKOU8.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU9.Text))
            {
                this.dto.entryEntity.MOD_BIKOU9 = this.form.MOD_BIKOU9.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU10.Text))
            {
                this.dto.entryEntity.MOD_BIKOU10 = this.form.MOD_BIKOU10.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU11.Text))
            {
                this.dto.entryEntity.MOD_BIKOU11 = this.form.MOD_BIKOU11.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU12.Text))
            {
                this.dto.entryEntity.MOD_BIKOU12 = this.form.MOD_BIKOU12.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU13.Text))
            {
                this.dto.entryEntity.MOD_BIKOU13 = this.form.MOD_BIKOU13.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU14.Text))
            {
                this.dto.entryEntity.MOD_BIKOU14 = this.form.MOD_BIKOU14.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU15.Text))
            {
                this.dto.entryEntity.MOD_BIKOU15 = this.form.MOD_BIKOU15.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU16.Text))
            {
                this.dto.entryEntity.MOD_BIKOU16 = this.form.MOD_BIKOU16.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU17.Text))
            {
                this.dto.entryEntity.MOD_BIKOU17 = this.form.MOD_BIKOU17.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU18.Text))
            {
                this.dto.entryEntity.MOD_BIKOU18 = this.form.MOD_BIKOU18.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU19.Text))
            {
                this.dto.entryEntity.MOD_BIKOU19 = this.form.MOD_BIKOU19.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU20.Text))
            {
                this.dto.entryEntity.MOD_BIKOU20 = this.form.MOD_BIKOU20.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU21.Text))
            {
                this.dto.entryEntity.MOD_BIKOU21 = this.form.MOD_BIKOU21.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU22.Text))
            {
                this.dto.entryEntity.MOD_BIKOU22 = this.form.MOD_BIKOU22.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU23.Text))
            {
                this.dto.entryEntity.MOD_BIKOU23 = this.form.MOD_BIKOU23.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU24.Text))
            {
                this.dto.entryEntity.MOD_BIKOU24 = this.form.MOD_BIKOU24.Text;
            }
            if (!string.IsNullOrEmpty(this.form.MOD_BIKOU25.Text))
            {
                this.dto.entryEntity.MOD_BIKOU25 = this.form.MOD_BIKOU25.Text;
            }
            //quoc-end
            // 引合取引先ﾌﾗｸﾞ 
            ...
            switch (this.form.WindowType)
            {
                ...
                case WINDOW_TYPE.DELETE_WINDOW_FLAG:
                    ...
                    //Trong trường hợp cập nhật hoặc xóa, không cập nhật thông tin tạo trước đó
        ...
        /// Tạo CreateDetailEntity để đăng ký từ điều khiển
        /// </summary>
        ...
        internal bool Print()
        {
            ...
            try
            {
                ...
                //quoc-begin
                string layoutName1 = string.Empty;
                string projectId1 = string.Empty;
                //quoc-end


                // 帳票宣言 (Khai báo biểu mẫu)
                ReportInfoR425_R508_R547_R548 reportInfo = new ReportInfoR425_R508_R547_R548(WINDOW_ID.R_MITSUMORISYO);
                //quoc-begin
                ReportInfoR999999 reportInfo1 = new ReportInfoR999999(WINDOW_ID.R_MITSUMORISYO);
                if (this.form.MOD_FAX_SOUFUJOU_INJI.Text == "1")
                { 
                    reportInfo1.OutputType = ReportInfoR999999.OutputTypeDef.TankaMitsumoriV;
                    layoutName1 = "LAYOUT3";
                    projectId1 = "R547";
                }
                //quoc-end
                ...
                //quoc-begin
                if (!string.IsNullOrEmpty(this.dto.torihikisakiEntry.TORIHIKISAKI_CD))
                {
                    var torihikiEnity = this.dto.torihikisakiEntry;
                    var todoufuken = this.accessor.GetDataByCodeForToDouFuKen(torihikiEnity.TORIHIKISAKI_TODOUFUKEN_CD.ToString());
                    if (todoufuken != null && todoufuken.Length > 0)
                    {
                        this.dto.todoufukenEnity = todoufuken[0];
                    }
                }
                //quoc-end

                //　印字拠点1情報(in thông tin cơ sở 1)
                ...
                // 引渡すデータ(dữ liệu sẽ được phân phối)
                DataTable reportData = new DataTable();

                // ページ分データ作成(tạo dữ liệu trang)
                for (int page = 1; page <= this.form.tbc_MitsumoriMeisai.TabPages.Count; page++)
                {
                    ...
                    //quoc-begin
                    reportInfo1.DataTablePageList[page.ToString()] = new Dictionary<string, DataTable>();
                    reportInfo1.DataTablePageList[page.ToString()]["Header"] = this.getPageHeader(this.dto, page);
                    reportInfo1.DataTablePageList[page.ToString()]["Detail"] = this.getPageDetail(page);
                    reportInfo1.DataTablePageList[page.ToString()]["Footer"] = this.getPageFooter(page);
                    //quoc-end
                }
                ...
                // 現在表示されている一覧をレポート情報として生成(tạo danh sách hiện đang hiển thị dưới dạng thông tin báo cáo)
                ...
                using (FormReport reportPopup = new FormReport(reportInfo, projectId, WINDOW_ID.R_MITSUMORISYO))
                {
                    ...
                    // 印刷設定の取得(thu nhận cài đặt in)
                    //reportPopup.SetPrintSetting(MitumorisyoConst.MITSUMORISHO);

                    // 印刷アプリ初期動作(プレビュー)
                    //Thao tác ban đầu của ứng dụng in (xem trước)
                ...
                if (this.form.MOD_FAX_SOUFUJOU_INJI.Text == "1")
                {
                    reportInfo1.Create(@".\Template\R999999-Form.xml", layoutName1, new DataTable());
                    using (FormReport reportPopup = new FormReport(reportInfo1, projectId1, WINDOW_ID.R_MITSUMORISYO))
                    {
                        reportPopup.Caption = "FAX送信票";
                        //Thao tác ban đầu của ứng dụng in (xem trước)
                        reportPopup.PrintInitAction = 2;
        ...
        private DataTable getPageHeader(DTOClass dto, int pageNo)
        {
            ...
            for (int iHeader = 0; iHeader < headerReportItemName.Length; iHeader++)
            {
                ...
                //quoc-sheet8.3-begin
                if (headerReportItemName[iHeader].Equals("MOD_SHAIN_TEL"))
                {
                    PropertyUtility.GetValue(this.dto.entryEntity, "SHAIN_CD", out value);
                    if (value == "")
                    {
                        value = String.Empty;
                    }
                    ...
                    {
                        M_SHAIN entShain = shainDao.GetDataByCd(Convert.ToString(value));
                        value = entShain.TXT_TEL;
                        value = "TEL " + value;
                ...
                if (headerReportItemName[iHeader].Equals("MITSUMORI_DATE"))
                {
                    ...
                    if ("True".Equals(valueInji.ToString()))
                    {
                        ...
                        //quoc-sheet3.1,sheet14.1-begin
                        if (DateTime.TryParse(value.ToString(), out dt))
                            {
                                strdate = dt.ToString("yyyy年MM月dd日");
                            }
                        
                        value = strdate;
                        //quoc-sheet3.1,sheet14.1-end
                        if (value == null || "Null".Equals(value.ToString()))
                        {
                            value = String.Empty;
                        }
                    ...
                    {
                        //quoc-sheet3.1-begin
                        value = string.Format("    年  月  日", valueInji);
                        
                        //string.ToString("    [年]  [月]  [日]");
                        //quoc-sheet3.1-end
                    }
                }
                ...
                if (headerReportItemName[iHeader].Equals("MOD_KAKUIN_INJI"))
                {
                    PropertyUtility.GetValue(this.dto.entryEntity, "MOD_KAKUIN_INJI", out value);
                    value = value == null ? "" : value;
                }

                if (headerReportItemName[iHeader].Equals("MOD_GOUKEI_KINGAKU_INJI"))
                {
                    PropertyUtility.GetValue(this.dto.entryEntity, "MOD_GOUKEI_KINGAKU_INJI", out value);
                    value = value == null ? "" : value;
                }


                if (headerReportItemName[iHeader].Equals("MOD_INEI_DATA"))
                {
                    PropertyUtility.GetValue(this.dto.shainEnity, "MOD_INEI_DATA", out value);
                    value = value == null ? "" : value;
                }
                ...
                if (!string.IsNullOrEmpty(this.dto.torihikisakiEntry.TORIHIKISAKI_CD))
                {
                    if (headerReportItemName[iHeader].Equals("TODOUFUKEN_NAME"))
                    {
                        value = this.dto.todoufukenEnity.TODOUFUKEN_NAME == null ? "" : this.dto.todoufukenEnity.TODOUFUKEN_NAME;
                    }
                }

                if (this.dto.torihikisakiEntry != null)
                {
                    if (headerReportItemName[iHeader].Equals("TORIHIKISAKI_BUSHO_NAME"))
                    {
                        value = this.dto.torihikisakiEntry.BUSHO == null ? "" : this.dto.torihikisakiEntry.BUSHO;
                    }
                    if (headerReportItemName[iHeader].Equals("TORIHIKISAKI_TEL"))
                    {
                        value = this.dto.torihikisakiEntry.TORIHIKISAKI_TEL == null ? "" : this.dto.torihikisakiEntry.TORIHIKISAKI_TEL;
                    }
                    if (headerReportItemName[iHeader].Equals("TORIHIKISAKI_NAME1"))
                    {
                        value = this.dto.torihikisakiEntry.TORIHIKISAKI_NAME1 == null ? "" : this.dto.torihikisakiEntry.TORIHIKISAKI_NAME1;
                    }
                    if (headerReportItemName[iHeader].Equals("TORIHIKISAKI_NAME2"))
                    {
                        value = this.dto.torihikisakiEntry.TORIHIKISAKI_NAME2 == null ? "" : this.dto.torihikisakiEntry.TORIHIKISAKI_NAME2;
                    }
                    if (headerReportItemName[iHeader].Equals("TORIHIKISAKI_ADDRESS1"))
                    {
                        value = this.dto.torihikisakiEntry.TORIHIKISAKI_ADDRESS1 == null ? "" : this.dto.torihikisakiEntry.TORIHIKISAKI_ADDRESS1;
                    }
                    if (headerReportItemName[iHeader].Equals("TORIHIKISAKI_ADDRESS2"))
                    {
                        value = this.dto.torihikisakiEntry.TORIHIKISAKI_ADDRESS2 == null ? "" : this.dto.torihikisakiEntry.TORIHIKISAKI_ADDRESS2;
                    }

                    if (headerReportItemName[iHeader].Equals("TANTOUSHA"))
                    {
                        value = this.dto.torihikisakiEntry.TANTOUSHA == null ? "" : this.dto.torihikisakiEntry.TANTOUSHA;
                    }
                }

                if (headerReportItemName[iHeader].Equals("TENWABANGOU"))
                {
                    value = this.dto.entryEntity.MOD_TENWABANGOU == null ? "" : this.dto.entryEntity.MOD_TENWABANGOU;
                }
                if (this.dto.shainEnity != null)
                {
                    if (headerReportItemName[iHeader].Equals("MAIL_ADDRESS"))
                    {
                        value = this.dto.shainEnity.MAIL_ADDRESS == null ? "" : this.dto.shainEnity.MAIL_ADDRESS;
                    }
                }
                if (headerReportItemName[iHeader].Equals("KENMEI"))
                {
                    PropertyUtility.GetValue(this.dto.entryEntity, "KENMEI", out value);
                    value = value == null ? "" : value;
                }

                if (headerReportItemName[iHeader].Equals("MOD_BIKOU1"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU2"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU3"))
                {
                  
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU4"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU5"))
                {
                  
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU6"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU7"))
                {
                
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU8"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU9"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU10"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU11"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU12"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU13"))
                {
                  
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU14"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU15"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU16"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU17"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU18"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU19"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU20"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU21"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU22"))
                {
                    
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU23"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU24"))
                {
                   
                    value = value == null ? "" : value;
                }
                if (headerReportItemName[iHeader].Equals("MOD_BIKOU25"))
                {
        ...
        private DataTable getPageDetail(int pageNo)
        {
            ...
            // 数量フォーマット取得(Lấy định dạng số lượng)
            string suuryouFormatStr = this.mSysInfo.SYS_SUURYOU_FORMAT;
            // 単価フォーマット取得(Lấy định dạng đơn giá)
            ...
            // CreateDataTableForEntityがMultiRowを動的に作成しないため、ここでEntity分行数を追加する
            // Entity数Rowを作ると最終行が無いので、Etity + 1でループさせる
            //Vì không có hàng cuối cùng khi tạo hàng cho số lượng thực thể, hãy lặp với Etity + 1
                ...
                if (pageNumber == SqlInt16.Parse((pageNo).ToString()))
                {
                    ...
                    for (int iDetail = 0; iDetail < detailReportItemName.Length; iDetail++)
                    {
                        ...
                        if (detailReportItemName[iDetail].Equals("DENPYOU_NUMBER"))
                        {
                            ...
                            //edit
                            if (this.dto.detailEntity[row].HINMEI_CD.IsNullOrEmpty())
                            {
                                value = string.Empty;
                            }
                            else
                        ...
                        if (detailReportItemName[iDetail].Equals("HINMEI_NAME"))
                        {
                            ...
                            if (this.dto.detailEntity[row].HINMEI_CD.IsNullOrEmpty())
                            {
                                //quoc-begin
                                value = string.Empty;
                                this.dto.detailEntity[row].SUURYOU = SqlDecimal.Null;
                                this.dto.detailEntity[row].UNIT_CD = SqlInt16.Null;
                                this.dto.detailEntity[row].TANKA = SqlDecimal.Null;
                                this.dto.detailEntity[row].KINGAKU = SqlDecimal.Null;
                                //quoc-end
                            }
        ...
        internal void CalcTanka(DataGridViewRow targetRow)
        {
            ...
            //Cài đặt phản hồi tạm thời (nhà cung cấp, địa chỉ null) *vì không có mẫu
            gyoushaC = string.Empty;
            genbaCD = string.Empty;
            // -------------------------------------------------------------
            // 個別品名単価から取得
            // Thu được từ đơn giá của tên sản phẩm riêng lẻ
            ...
            //Nếu không thể lấy thông tin từ đơn giá tên sản phẩm riêng lẻ, hãy tìm kiếm đơn giá tên sản phẩm cơ bản.
        ...
        public bool Torihikisaki_GyoushaCD_GenbaCD_LostFocus(string torihikisakiCode, string gyoushaCD, string genbaCD, out bool catchErr)
        {
            ...
            {
                // 取引先ロストフォーカスイベント
                //Sự kiện mất tiêu điểm tài khoản
        ...
        public bool TorihikisakiLostFocus(string torihikisakiCode)
        {
            ...
            //Chuyển đổi giữa hiển thị yêu cầu hiện có hoặc hiển thị
        ...
        public bool GenbaCD_LostFocus(string gyoushaCD, string genbaCD)
        {
            ...
            if (!string.IsNullOrEmpty(this.form.txt_GenbaCD.Text) && string.IsNullOrEmpty(this.form.txt_GyoushaCD.Text))
            {
                ...
                // 現場タブのコントロールを使用不可
                //Điều khiển tab trường bị vô hiệu hóa
                foreach (System.Windows.Forms.Control ctl in this.form.tap_Genba.Controls)
                {
                    if (ctl is System.Windows.Forms.Label)
                    {
                ...
                // エラー区分＝１：業者エラー
                //Phân loại lỗi = 1: Lỗi nhà cung cấp
        ...
        public void setDataTorihikisakiTab(bool NameBasedChangeFlg)
        {
            ...
            switch (HIKIAI_TORIHIKISAKI_WindowType)
            {
                ...
                case HIKIAI_TORIHIKISAKI.REFERENCE_WINDOW_FLAG:
                    ...
                    // 取引先タブのコントロールを使用不可
                    //Vô hiệu hóa kiểm soát trên tab tài khoản
                    foreach (System.Windows.Forms.Control ctl in this.form.tap_Torihikisaki.Controls)
                    {
                        if (ctl is System.Windows.Forms.Label)
                        {
        ...
        /// <summary>
        /// 現場タブ画面表示設定
        /// Cài đặt hiển thị màn hình tab trường
        /// </summary>
        public void setDataGenbaTab(bool NameBasedChangeFlg)
        {
            ...
            {
                // 名称を再取得するか判定
                //Xác định xem có lấy lại tên hay không
                if (this.dto.genbaEntry != null && this.dto.genbaEntry.GENBA_CD != null)
                {
                    // CDが変更された場合かつ諸口で名称が変更されていない場合
                    //Nếu đĩa CD đã thay đổi và tên không thay đổi ở nhiều nơi
        ...
        /// <summary>
        /// 初期の場合、明細税区分を制御
        /// Nếu ban đầu, chỉ số thuế dòng kiểm soát
        /// </summary>
        ...
        internal bool CheckTorihikisaki(out bool catchErr)
        {
            ...
            try
            {
                ...
                // 取引先CDの空チェック
                //Kiểm tra trống đĩa CD của khách hàng
                        ...
                        if (!this.form.hikiaiDisplaySwitchingFlg)
                        {
                            ...
                            if (null != torihikisakiEntity)
                            {
                                ...
                                if (toriCheck)
                                {
                                    ...
                                    //quoc-sheet4.1-begin
                                    if (!string.IsNullOrEmpty(torihikisakiEntity.TORIHIKISAKI_KEISHOU1) && !string.IsNullOrEmpty(torihikisakiEntity.TORIHIKISAKI_KEISHOU2))
                                    {
                                        this.form.lb_TorihikisakiKeishou.Text = torihikisakiEntity.TORIHIKISAKI_KEISHOU2;
                                    }
                                    else if (!string.IsNullOrEmpty(torihikisakiEntity.TORIHIKISAKI_KEISHOU1))
                                    {
                                        this.form.lb_TorihikisakiKeishou.Text = torihikisakiEntity.TORIHIKISAKI_KEISHOU1;
                                    }
                                    else
                                    {
                                        this.form.lb_TorihikisakiKeishou.Text = "";
                                    }
                                    //quoc-sheet4.1-end
                ...
                else
                {
                    ...
                    {
                        // 取引先CDが空の場合、関連項目クリア
                        //Nếu đĩa CD tài khoản trống, hãy xóa Xem thêm
                        this.form.txt_TorihikisakiMei.Text = string.Empty;
                        //quoc-begin
                        this.form.lb_TorihikisakiKeishou.Text = string.Empty;
                        //quoc-end
                    ...
                    if (gyoushaDisplaySwitching())
                    {
                        ...
                        if (!this.form.hikiaiDisplaySwitchingFlg)
                        {
                            ...
                            if (null != gyoushaEntity)
                            {
                                ...
                                //quoc-sheet4.1-begin
                                if (!string.IsNullOrEmpty(gyoushaEntity.GYOUSHA_KEISHOU1) && !string.IsNullOrEmpty(gyoushaEntity.GYOUSHA_KEISHOU2))
                                {
                                    this.form.lb_GyoushaKeishou.Text = gyoushaEntity.GYOUSHA_KEISHOU2;
                                }
                                else if (!string.IsNullOrEmpty(gyoushaEntity.GYOUSHA_KEISHOU1))
                                {
                                    this.form.lb_GyoushaKeishou.Text = gyoushaEntity.GYOUSHA_KEISHOU1;
                                }
                                else
                                {
                                    this.form.lb_GyoushaKeishou.Text = "";
                                }
                                //quoc-sheet4.1-end
                        ...
                        if (!this.form.hikiaiDisplaySwitchingFlg)
                        {
                            ...
                            foreach (M_GENBA genbaEntity in genbaEntityList)
                            {
                                if (this.form.txt_GyoushaCD.Text.Equals(genbaEntity.GYOUSHA_CD))
                                {
                                    ...
                                    //quoc-sheet4.1-begin
                                    if (!string.IsNullOrEmpty(genbaEntity.GENBA_KEISHOU1) && !string.IsNullOrEmpty(genbaEntity.GENBA_KEISHOU2))
                                    {
                                        this.form.lb_GenbaKeishou.Text = genbaEntity.GENBA_KEISHOU2;
                                    }
                                    else if (!string.IsNullOrEmpty(genbaEntity.GENBA_KEISHOU1))
                                    {
                                        this.form.lb_GenbaKeishou.Text = genbaEntity.GENBA_KEISHOU1;
                                    }
                                    else
                                    {
                                        this.form.lb_GenbaKeishou.Text = "";
                                    }
                                    //quoc-sheet4.1-end
        ...
        internal bool CheckTorihikisakiAndKyotenCd(M_HIKIAI_TORIHIKISAKI torihikisakiEntity_hiki, M_TORIHIKISAKI torihikisakiEntity, string TorihikisakiCd, out bool catchErr)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        // 取引先の入力がない場合はチェック対象外
                        //Nếu không có đầu vào của đối tác kinh doanh, nó sẽ không được chọn
                        returnVal = true;
                        return returnVal;
                    }
        ...
        internal bool torihikisakiDisplaySwitching()
        {
            ...
            // 初期値：既存表示に設定
            ////Giá trị ban đầu: Đặt thành hiển thị hiện tại
            this.form.hikiaiDisplaySwitchingFlg = false;


            // 手入力の場合はマスタの存在チェックで判断する
            //Trong trường hợp nhập thủ công, nó được xác định bằng cách kiểm tra sự tồn tại của bản gốc
        ...
        internal bool MitsumoriSyuruiCheck()
        {
            ...
            try
            {
                ...
                //Chuyển đổi loại trích dẫn CD thành formParem.cyohyoType
                this.form.formParem.cyohyoType = this.ConvertCyohyoType(this.form.MITSUMORISYURUI_CD.Text);

                // 全てのタブに対して変更をかける
                //thay đổi tất cả các tab
                for (int page = 0; page < this.form.tbc_MitsumoriMeisai.TabPages.Count; page++)
                {
                    // 明細欄のプロパティを変更
                    //Thay đổi thuộc tính mục hàng
            ...
            if (this.form.formParem != null)
            {
                ...
                switch (this.form.formParem.cyohyoType)
                {
                    case MitumorisyoConst.MITUMOTISYO_KINGAKU_TATE: // 見積書（縦）
                        ...
                        //quoc-sheet4.5-begin
                        //this.form.MOD_GOUKEI_KINGAKU_INJI.ReadOnly = false;
                        //this.form.MOD_RDO_ZEIKOMI.Enabled = true;
                        //this.form.MOD_RDO_ZEINUKI.Enabled = true;
                        Enable_GOUKEI_KINGAKU_INJI(false, true);
                        //quoc-sheet4.5-end
                        break;
                    case MitumorisyoConst.MITUMOTISYO_KINGAKU_YOKO: // 見積書（横）
                        this.form.MITSUMORISYURUI_NAME.Text = MitumorisyoConst.MITUMOTISYO_KINGAKU_YOKO_NAME;
                        this.form.MITSUMORISYURUI_CD.Text = MitumorisyoConst.MITUMOTISYO_KINGAKU_YOKO_INDEX.ToString();
                        Enable_GOUKEI_KINGAKU_INJI(true, false);
                        break;
                    case MitumorisyoConst.MITUMOTISYO_TANKA_TATE:   // 単価見積書（縦）
                        this.form.MITSUMORISYURUI_NAME.Text = MitumorisyoConst.MITUMOTISYO_TANKA_TATE_NAME;
        ...
        //quoc-begin
        public void Enable_GOUKEI_KINGAKU_INJI(bool value1, bool value2)
        {
            this.form.MOD_GOUKEI_KINGAKU_INJI.ReadOnly = value1;
            this.form.MOD_RDO_ZEIKOMI.Enabled = value2;
            this.form.MOD_RDO_ZEINUKI.Enabled = value2;
        }
        //quoc-end
        ...
        public void MitsumoriSyuruiPopUpDataInit()
        {
            try
            {
                ...
                //Nếu bạn đặt TableName thì nó sẽ là tiêu đề của popup
                mitsumoriSyuruiDataTable.TableName = "見積書種類選択";

                // 列名とデータソース設定
                //Tên cột và cài đặt nguồn dữ liệu
