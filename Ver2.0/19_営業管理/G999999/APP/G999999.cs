...
namespace Shougun.Core.BusinessManagement.Soushinpyoubikou
{
    ...
    /// G276 送信票備考
    /// </summary>
    class G999999 : r_framework.FormManager.IShougunForm
    {
        /// <summary>
        /// フォーム作成
        /// </summary>
        ...
        public Form CreateForm(params object[] args)
        {
            ...
            // 引数 arg[3] :業者CD
            // 引数 arg[4] :現場CD
            // Đối số arg[2] : CD khách hàng
            // Đối số arg[3] : Nhà cung cấp CD
            // Đối số arg[4] : Trang web CD
            SoushinpyoubikouForm callForm = null;
            //データ移動モード判定
            if (windowType == WINDOW_TYPE.NEW_WINDOW_FLAG &&
                args.Length > 4 &&
                args[1] == null)
            {
                callForm = new Shougun.Core.BusinessManagement.Soushinpyoubikou.SoushinpyoubikouForm(WINDOW_ID.T_SOUSHINPYOUBIKOU, windowType, formParem, null,
                                                                                                    (string)args[2], (string)args[3], (string)args[4]);
            }
            else
            {
                callForm = new Shougun.Core.BusinessManagement.Soushinpyoubikou.SoushinpyoubikouForm(WINDOW_ID.T_SOUSHINPYOUBIKOU, windowType, formParem);
            ...
            if (args.Length > 1)
            {
                ...
                if (args[1] != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(formParem.mitsumoriNumber))
                    {
                        ...
                        var uiForm = footerForm.inForm as SoushinpyoubikouForm;
