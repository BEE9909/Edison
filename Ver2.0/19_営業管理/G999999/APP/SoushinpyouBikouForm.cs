// $Id: SoushinpyoubikouForm.cs 53608 2015-06-26 00:09:57Z miya@e-mall.co.jp $
using System;
using System.Text;
...
{
    /// <summary>
    /// 送信票備考画面E:\SVN\trunk_1\19_営業管理\G276\APP\SoushinpyoubikouForm.cs
    /// </summary>
    [Implementation]
    public partial class SoushinpyoubikouForm : SuperForm
    {
        ...
        /// 画面ロジック
        /// </summary>
        private Shougun.Core.BusinessManagement.Soushinpyoubikou.LogicClass logic;
        ...
        /// <param name="lastRunMethod">送信票備考で閉じる前に実行するメソッド(別画面からの遷移用)</param>
        public SoushinpyoubikouForm(WINDOW_ID windowId, WINDOW_TYPE windowType, FormShowParamDao formParem, LastRunMethod lastRunMethod = null)
            : base(WINDOW_ID.T_SOUSHINPYOUBIKOU, windowType)
        {
        ...
        public SoushinpyoubikouForm(WINDOW_ID windowId, WINDOW_TYPE windowType, FormShowParamDao formParem, LastRunMethod lastRunMethod,
        ...
        private void SoushinpyoubikouForm_Shown(object sender, EventArgs e)
        {
            // データ移動初期表示処理
        ...
        private void txt_Bikou1_KeyPress(object sender, KeyPressEventArgs e)
        {
            //// Số dòng tối đa được nhập là 8
            //int maxLines = 8;
            ////Backspace` (`\b`) hoặc `Delete` (`\x7F`)

            //// Không cho phép nhập khi số dòng đã đạt đến giới hạn tối đa
            //if (txt_Bikou1.Lines.Length == maxLines && e.KeyChar == (char)Keys.Enter)
            //{
            //    e.Handled = true;
            //    if (e.KeyChar == '\r' || e.KeyChar == '\b' || e.KeyChar == '\x7F')
            //    {
            //        e.Handled = false;
            //    }
            //}
            //// Kiểm tra số dòng hiện tại có vượt quá giới hạn tối đa hay không
            //else if (txt_Bikou1.Lines.Length > maxLines)
            //{
            //    e.Handled = true;
            //    if (e.KeyChar == '\r' || e.KeyChar == '\b' || e.KeyChar == '\x7F')
            //    {
