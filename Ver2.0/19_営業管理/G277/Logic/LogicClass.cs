...
namespace Shougun.Core.BusinessManagement.MitumoriIchiran
{
    ...
    public class LogicClass : IBuisinessLogic
    {
        ...
        //quoc-begin
        internal readonly string MOD_CELL_SHOUNIN = "承認";
        internal readonly string MOD_CELL_SHOUNIN_STATUS = "承認ステータス";
        internal readonly string MOD_CELL_SHOUNIN_NICHIJI = "承認日時";
        DateTime now = DateTime.Now; // tạo đối tượng DateTime mới, lưu trữ ngày giờ hiện tại
        ...
        private void EventInit()
        {
            try
            {
                ...
                parentForm.bt_process3.Click += new EventHandler(bt_process3_Click);
                //quoc-end
                //画面上でESCキー押下時のイベント生成
        ...
        private void bt_func3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // 引数遷移先パラメータ設定dto
                // Cài đặt tham số đích chuyển đổi đối số dto
                ...
                if (datagridviewcell.RowIndex >= 0)
                {
                    ...
                    //quoc-begin
                    formParem.MOD_STATUS = this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.MOD_CELL_SHOUNIN_STATUS].Value.ToString();
                    formParem.MOD_SHOUNIN_NICHIJI = this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.MOD_CELL_SHOUNIN_NICHIJI].Value.ToString();
                    //quoc-end
                }
        ...
        private void bt_func4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (datagridviewcell.RowIndex >= 0)
                {
                    // 見積番号設定 Cài đặt số báo giá 
                    formParem.mitsumoriNumber = this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.HIDDEN_MITUSMORI_NUMBER].Value.ToString();
                    // 帳票種別設定 cài đặt loại báo cáo
                    formParem.cyohyoType = int.Parse(this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.HIDDEN_MITSUMORI_SHOSHIKI_KBN].Value.ToString());
                }

                // 削除権限がある場合入力画面表示
                //Hiển thị màn hình nhập nếu bạn có quyền xóa
        ...
        private void bt_func8_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //quoc-sheet-begin
                bool isErr2 = this.mitsumoriBangoCheck(out catchErr);
                //quoc-sheet-end
                if (catchErr || isErr )
                {
                    return;
        ...
        private void bt_process2_Click(object sender, System.EventArgs e)
        {
            try
            {
                LogUtility.DebugMethodStart(sender, e);
                var parentForm = (BusinessBaseForm)this.myForm.Parent;
                if (Manager.CheckAuthority("G277", WINDOW_TYPE.REFERENCE_WINDOW_FLAG, false))
                {
                    parentForm.bt_func3.Enabled = true;
                    DataGridViewCell datagridviewcell = this.myForm.customDataGridView1.CurrentCell;
                    string dateString = now.ToString("yyyy/MM/dd HH:mm");
                    if (this.myForm.MOD_SHOUNIN_STATUS.Text == "1" || this.myForm.MOD_SHOUNIN_STATUS.Text == "3")
                    {
                        this.myForm.MOD_SHOUNIN_STATUS.Text = "2";
                        this.myForm.MOD_SHINSEI_NAKA.Enabled = false;
                        if (datagridviewcell != null)
                        {
                            if (this.myForm.customDataGridView1.DataSource != null)
                            {
                                DataTable dtSource = this.myForm.customDataGridView1.DataSource as DataTable;
                                dtSource.ToString();

                                if (this.myForm.customDataGridView1.Columns.Contains(this.MOD_CELL_SHOUNIN_STATUS))
                                {
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_STATUS].ReadOnly = false;
                                    this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.MOD_CELL_SHOUNIN_STATUS].Value = "承認";
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_STATUS].ReadOnly = true;
                                }
                                if (this.myForm.customDataGridView1.Columns.Contains(this.MOD_CELL_SHOUNIN_NICHIJI))
                                {
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_NICHIJI].ReadOnly = false;
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_NICHIJI].MaxLength = -1;
                                    this.myForm.customDataGridView1.Columns[this.MOD_CELL_SHOUNIN_NICHIJI].Width = 121;
                                    this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.MOD_CELL_SHOUNIN_NICHIJI].Value = dateString;
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_NICHIJI].ReadOnly = true;
                                }  
                            }
                        }
                    }
        ...
        private void bt_process3_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (Manager.CheckAuthority("G277", WINDOW_TYPE.REFERENCE_WINDOW_FLAG, false))
                {
                    ...
                    DataGridViewCell datagridviewcell = this.myForm.customDataGridView1.CurrentCell;
                    string dateString = now.ToString("yyyy/MM/dd HH:mm");
                    if (this.myForm.MOD_SHOUNIN_STATUS.Text == "1" || this.myForm.MOD_SHOUNIN_STATUS.Text == "2")
                    {
                        this.myForm.MOD_SHOUNIN_STATUS.Text = "3";
                        this.myForm.MOD_SHINSEI_NAKA.Enabled = false;
                        if (datagridviewcell != null)
                        {
                            if (this.myForm.customDataGridView1.DataSource != null)
                            {
                                ...
                                {
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_STATUS].ReadOnly = false;
                                    this.myForm.customDataGridView1.Rows[datagridviewcell.RowIndex].Cells[this.MOD_CELL_SHOUNIN_STATUS].Value = "否認";
                                    dtSource.Columns[this.MOD_CELL_SHOUNIN_STATUS].ReadOnly = true;
                                }
                                if (this.myForm.customDataGridView1.Columns.Contains(this.MOD_CELL_SHOUNIN_NICHIJI))
                                {
        ...
        #region 明細データダブルクリックイベント(Dữ liệu từng mục nhấp đúp vào sự kiện)
        ...
        private void customDataGridView1_MouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                //quoc-sheet13.2-begin
                formParem.MOD_STATUS = this.myForm.customDataGridView1.Rows[e.RowIndex].Cells[this.MOD_CELL_SHOUNIN_STATUS].Value.ToString();
                formParem.MOD_SHOUNIN_NICHIJI = this.myForm.customDataGridView1.Rows[e.RowIndex].Cells[this.MOD_CELL_SHOUNIN_NICHIJI].Value.ToString();
                //quoc-end
                // 修正権限チェック
                if (Manager.CheckAuthority("G276", WINDOW_TYPE.UPDATE_WINDOW_FLAG, false))
                {
        ...
        /// <summary>
        /// 見積一覧取得処理
        /// Quy trình thu thập danh sách báo giá
        /// </summary>
        ...
        public int Search()
        {
            ...
            try
            {
                ...
                // 検索文字列を作成して取得します
                //Tạo và truy xuất chuỗi tìm kiếm
        ...
        private string GetSearchString()
        {
            ...
            //quoc-sheet9.2-begin
            if (!string.IsNullOrEmpty(this.MitumoriKsjk.SICHU_DATE) && !string.IsNullOrEmpty(this.MitumoriKsjk.JUCHU_DATE))
            {
                
                sql.AppendFormat(" AND T_MITSUMORI_ENTRY.JUCHU_DATE  {0} ", this.MitumoriKsjk.JUCHU_DATE);

                sql.AppendFormat(" AND T_MITSUMORI_ENTRY.SICHU_DATE  {0} ", this.MitumoriKsjk.SICHU_DATE);

            }
            //sheet13.7
            if(!string.IsNullOrEmpty(this.MitumoriKsjk.MOD_SHOUNIN_STATUS)) 
            {
                sql.AppendFormat("AND T_MITSUMORI_ENTRY.MOD_TXT_STATUS = '{0}'",this.MitumoriKsjk.MOD_SHOUNIN_STATUS);
            }
            //sheet13.7
            if (!string.IsNullOrEmpty(this.MitumoriKsjk.MOD_SHOUNIN_NICHIJI_FROM))
            {
                
                sql.Append(" AND CONVERT(DATETIME, CONVERT(nvarchar, T_MITSUMORI_ENTRY.MOD_SHOUNIN_NICHIJI, 111), 120) >= ");
                sql.AppendFormat("CONVERT(DATETIME, CONVERT(nvarchar, '{0}', 111), 120) ", this.MitumoriKsjk.MOD_SHOUNIN_NICHIJI_FROM);
            }
            if (!string.IsNullOrEmpty(this.MitumoriKsjk.MOD_SHOUNIN_NICHIJI_TO))
            {
                sql.Append(" AND CONVERT(DATETIME, CONVERT(nvarchar, T_MITSUMORI_ENTRY.MOD_SHOUNIN_NICHIJI, 111), 120) <= ");
                sql.AppendFormat("CONVERT(DATETIME, CONVERT(nvarchar, '{0}', 111), 120) ", this.MitumoriKsjk.MOD_SHOUNIN_NICHIJI_TO);
            }
            //quoc-end
            ...
            #endregion
            string chuoi = sql.ToString();
            return sql.ToString();
        }

        /// <summary>
        /// 検索条件設定
        /// Cài đặt điều kiện tìm kiếm
        /// </summary>
        private void SetSearchCondition()
        {
            ...
            //quoc-sheet9.2-begin
            if(this.myForm.txtNum_Jyoukyou.Text == "3")
            {
                this.MitumoriKsjk.JUCHU_DATE = "IS NULL";
                this.MitumoriKsjk.SICHU_DATE = "IS NULL";
            }
            
            if(this.myForm.MOD_SHOUNIN_STATUS.Text == "1")
            {
                this.MitumoriKsjk.MOD_SHOUNIN_STATUS = "申請中";
            }
            if (this.myForm.MOD_SHOUNIN_STATUS.Text == "2")
            {
                this.MitumoriKsjk.MOD_SHOUNIN_STATUS = "承認";
            }
            if (this.myForm.MOD_SHOUNIN_STATUS.Text == "3")
            {
                this.MitumoriKsjk.MOD_SHOUNIN_STATUS = "否認";
            }
            if (!string.IsNullOrEmpty(this.myForm.MOD_MITSUMORI_BANGO_FROM.Text))
            {
                this.MitumoriKsjk.MOD_SHOUNIN_NICHIJI_FROM = this.myForm.MOD_MITSUMORI_BANGO_FROM.Text.Substring(0, 10);
            }
            if (!string.IsNullOrEmpty(this.myForm.MOD_MITSUMORI_BANGO_TO.Text))
            {
                this.MitumoriKsjk.MOD_SHOUNIN_NICHIJI_TO = this.myForm.MOD_MITSUMORI_BANGO_TO.Text.Substring(0, 10);
            }
        ...
        internal bool mitsumoriBangoCheck(out bool catchErr)
        {
            catchErr = false;
            bool isErr = false;
            try
            {
                ...
                // 日付FROM > 日付TO 場合
                if (!string.IsNullOrEmpty(this.myForm.MOD_MITSUMORI_BANGO_FROM.Text.Trim()) &&
                    !string.IsNullOrEmpty(this.myForm.MOD_MITSUMORI_BANGO_TO.Text.Trim()))
                {
                    if (this.myForm.MOD_MITSUMORI_BANGO_TO.Text.Trim().CompareTo(this.myForm.MOD_MITSUMORI_BANGO_FROM.Text.Trim()) < 0)
                    {
                        // koukouei 20141020 バグ 「From　>　To」のアラート表示タイミング変更 start
                        this.myForm.MOD_MITSUMORI_BANGO_FROM.IsInputErrorOccured = true;
                        this.myForm.MOD_MITSUMORI_BANGO_TO.IsInputErrorOccured = true;
                        this.myForm.MOD_MITSUMORI_BANGO_FROM.BackColor = Constans.ERROR_COLOR;
                        this.myForm.MOD_MITSUMORI_BANGO_TO.BackColor = Constans.ERROR_COLOR;
                        isErr = true;
                        string errorMsg = "伝票番号の設定を見直してください";
                        MessageBoxUtility.MessageBoxShowError(errorMsg);
                        this.myForm.MOD_MITSUMORI_BANGO_FROM.Focus();
                        LogUtility.DebugMethodEnd(isErr, catchErr);
                        return isErr;
                        // koukouei 20141020 バグ 「From　>　To」のアラート表示タイミング変更 end
                    }
            ...
            {
                LogUtility.Fatal("mitsumoriBangoCheck", ex);
                MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                if (ex is SQLRuntimeException)
                {
                    msgLogic.MessageBoxShow("E093", "");
        ...
        private void DeleteJoukyo()
        {
            ...
            //２、３の項目上に移動
            this.myForm.radbtn_Jyutyuu.Location = new System.Drawing.Point(70, 0);
