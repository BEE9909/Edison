...
namespace r_framework.Logic
{
    ...
    public class AutoRegistCheckLogic
    {
        ...
        /// コンストラクタ
        /// </summary>
        /// <parameparam name="control">対象のコントロール(kiểm soát mục tiêu)</parameparam>
        /// <parameparam name="control">Formに紐付くコントロールのコレクション(Một tập hợp các điều khiển được liên kết với Biểu mẫu)</parameparam>
        ...
        /// 存在するかを判定するメソッド
        /// Quy trình kiểm tra được đặt trong tài sản tại thời điểm đăng ký là
        ///phương pháp để xác định nếu nó tồn tại
        /// </summary>
        /// <param name="allCustomControl">form上のすべてのコントロール配列</param>
        public virtual bool AutoRegistCheck()
        {
            // エラーメッセージリストクリア
            //Xóa danh sách thông báo lỗi
            ...
            foreach (var c in allCustomControl)
            {
                ...
                //Điều khiển ReadOnly không được chọn
        ...
        internal virtual bool AutoCheckMasterMethodForMultiRow(GcCustomMultiRow multiRow, SuperForm form)
        {
            ...
            for (int i = 0; i < multiRow.RowCount; i++)
            {
                ...
                if (Convert.ToString(((DataTable)multiRow.DataSource).Rows[i]["DELETE_FLG"]) == "True"
