    ...
    public partial class GenchakuJikanHoshuForm : SuperForm
    {
        ...
        public virtual void Search(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                /*quoc-begin*/
                //for (int i = 0; i < Ichiran.Rows.Count - 2; i++)
                //{

                //    Ichiran.Rows[i].Cells["MOD_HAIKI_NAME"].ReadOnly = true;
                //}

                foreach (DataGridViewRow row in this.Ichiran.Rows)
                {
                    row.Cells["MOD_NINI_TORI_FUKA"].ReadOnly = true;


                }
                //foreach (DataGridViewRow row in this.Ichiran.Rows)
                //{
                //    row.Cells["MOD_HAIKI_NAME"].ReadOnly = true;
        ...
        private void Ichiran_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            try
            {
                ...
                if (!DBNull.Value.Equals(Ichiran.Rows[e.RowIndex].Cells["MOD_WARIATE_JUN"].Value) &&
                    !"".Equals(Ichiran.Rows[e.RowIndex].Cells["MOD_WARIATE_JUN"].Value) &&
                           Ichiran.Columns[e.ColumnIndex].DataPropertyName.Equals(Const.GenchakuJikanHoshuConstans.MOD_WARIATE_JUN))
                {
                    string MOD_WARIATE_JUN = Ichiran.Rows[e.RowIndex].Cells["MOD_WARIATE_JUN"].Value.ToString();
                    bool catchErr = true;
                    bool isError = this.logic.DuplicationCheck2(MOD_WARIATE_JUN, out catchErr);
                    if (!catchErr)
                    {
                        return;
                    }
                    ...
                    {
                        MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                        msgLogic.MessageBoxShow("E022", "入力された割当順");
                        e.Cancel = true;
                        this.Ichiran.BeginEdit(false);
                        return;
                    }
               ...
               // MOD_RYUUBEI_KANZANKEISUU
                if (!DBNull.Value.Equals(Ichiran.Rows[e.RowIndex].Cells["MOD_RYUUBEI_KANZANKEISUU"].Value) &&
                    !"".Equals(Ichiran.Rows[e.RowIndex].Cells["MOD_RYUUBEI_KANZANKEISUU"].Value))
                {
                    string MOD_RYUUBEI_KANZANKEISUU = Ichiran.Rows[e.RowIndex].Cells["MOD_RYUUBEI_KANZANKEISUU"].Value.ToString();
                    bool catchErr = true;
                    if (!catchErr)
                    {
                        return;
                    ...
                    if ((Convert.ToDecimal(this.Ichiran.Rows[e.RowIndex].Cells["MOD_RYUUBEI_KANZANKEISUU"].Value)) <= 0)
                        
                    {
                        MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                        msgLogic.MessageBoxShow("Q001");
                        e.Cancel = true;
                        this.Ichiran.BeginEdit(false);
                        return;
                    }
        ...
        #region 現着時間CD入力チェック
        ///  Kiểm tra đầu vào đĩa CD thời gian đến
        ///SỰ ƯU TIÊN
        /// <summary>
        /// 現着時間CD入力チェック
        /// </summary>
        ...
        public void CONDITION_VALUE_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            try
            {
                ...
                /*quoc-begin*/
                if (this.CONDITION_DBFIELD.Text.Equals("MOD_WARIATE_JUN"))
                {
                    this.CONDITION_VALUE.MaxLength = 2;
                    this.CONDITION_VALUE.CharactersNumber = 2;
        ...
        /// Lưới → Sự kiện chuyển đổi DataTable
        /// </summary>
        ...
        private void Ichiran_CellParsing(object sender, DataGridViewCellParsingEventArgs e)
        {
            ...
            if ("".Equals(e.Value)) //空文字を入力された場合,Nếu bạn nhập một ký tự trống
            {
                e.Value = System.DBNull.Value;  //AllowDBNull=trueの場合は nullはNG DBNullはOK,null là NG DBNull là OK
                e.ParsingApplied = true; //後続の解析不要, không cần phân tích tiếp theo
            }
        ...
        /// <summary>
        /// 条件ポップアップ後の実行処理
        /// Quá trình thực thi sau khi bật lên điều kiện
        /// </summary>
        ...
        // quopc BENGIN
        private void Ichiran_CellMouseUp(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.RowIndex > -1)
            {    //Nếu đúng cột unten sha
                if (this.Ichiran.Columns[e.ColumnIndex].Name.Equals(Const.GenchakuJikanHoshuConstans.MOD_UNTEN_SHA))
                {
                    Ichiran.EndEdit();
                }
                //Nếu đúng cột MOD_NYUURYOKU_TANTOUSHA
                if (this.Ichiran.Columns[e.ColumnIndex].Name.Equals(Const.GenchakuJikanHoshuConstans.MOD_NYUURYOKU_TANTOUSHA))
                {
                    Ichiran.EndEdit();
                }
            }
            ...
            if (e.RowIndex > -1)
            {
                ...
                if ((this.Ichiran.Columns[e.ColumnIndex].Name.Equals(Const.GenchakuJikanHoshuConstans.MOD_UNTEN_SHA)))
                {
                    ...
                    if (!this.Ichiran.Rows[e.RowIndex].IsNewRow)
                        this.logic.SetWariateJunState(this.Ichiran[Const.GenchakuJikanHoshuConstans.MOD_UNTEN_SHA, e.RowIndex], this.Ichiran[Const.GenchakuJikanHoshuConstans.MOD_WARIATE_JUN, e.RowIndex]);
                }
                if ((this.Ichiran.Columns[e.ColumnIndex].Name.Equals(Const.GenchakuJikanHoshuConstans.MOD_NYUURYOKU_TANTOUSHA)))
                {
                    //Cài đặt active checkbox
                    if (!this.Ichiran.Rows[e.RowIndex].IsNewRow)
                        this.logic.SetNiniToriFuka(this.Ichiran[Const.GenchakuJikanHoshuConstans.MOD_NYUURYOKU_TANTOUSHA, e.RowIndex], this.Ichiran[Const.GenchakuJikanHoshuConstans.MOD_NINI_TORI_FUKA, e.RowIndex]);
                }
            }
        }
        // quocpc End 
