    ...
    internal class LogicClass : IBuisinessLogic
    {
        ...
        public bool WindowInit()
        {
            try
            {
                ...
                //前の表示条件を初期値に設定する
                //Đặt điều kiện hiển thị trước đó thành giá trị ban đầu
                ...
                //Đặt thông tin phụ và IME theo cột điều kiện đã chọn
        ...
        public bool DuplicationCheck(string genchaku_Time_Cd, out bool catchErr)
        {
            ...
            {
                // 画面で現着時間CD重複チェック
                // Thời gian đến kiểm tra sao chép đĩa CD trên màn hình
                ...
                // 検索結果で現着CD重複チェック
                //Kiểm tra trùng lặp các đĩa CD hiện có trong kết quả tìm kiếm
                //Kiểm tra cờ xóa
                ...
                // DBで現着CD重複チェック
                //Kiểm tra sao chép CD đến trong DB
        ...
        /*quoc-begin*/
        public bool DuplicationCheck2(string MOD_WARIATE_JUN, out bool catchErr)
        {
            LogUtility.DebugMethodStart(MOD_WARIATE_JUN);
            catchErr = true;
            try
            {
                // 画面で現着時間CD重複チェック
                ...
                for (int i = 0; i < this.form.Ichiran.Rows.Count - 1; i++)
                {
                    if (this.form.Ichiran.Rows[i].Cells[Const.GenchakuJikanHoshuConstans.MOD_WARIATE_JUN].Value.ToString().Equals(Convert.ToString(MOD_WARIATE_JUN)))
                    {
                        recCount++;
                    }
                }
                ...
                for (int i = 0; i < dtDetailList.Rows.Count; i++)
                {
                    if (MOD_WARIATE_JUN.Equals(dtDetailList.Rows[i][1].ToString()))
                    {
                        LogUtility.DebugMethodEnd(false, catchErr);
                        return false;
                    }
                ...
                // DBで現着CD重複チェック
                //Kiểm tra sao chép CD đến trong DB
                M_GENCHAKU_TIME entity = this.dao.GetDataByCd(MOD_WARIATE_JUN.ToString());
                //M_GENCHAKU_TIME entity = this.dao.GetDataByCd(genchaku_Time_Cd);
            ...
            {
                LogUtility.Error("DuplicationCheck2", ex1);
                this.form.errmessage.MessageBoxShow("E093", "");
                catchErr = false;
            }
            catch (Exception ex)
            {
                LogUtility.Error("DuplicationCheck2", ex);
                this.form.errmessage.MessageBoxShow("E245", "");
                catchErr = false;
            }
            ...
            //quoc_Begin
            //truyền trạng thái wariateJun
            internal bool SetWariateJunState(DataGridViewCell untenCell, DataGridViewCell wariateCell)
        {
            if (untenCell == null || wariateCell == null)
                wariateCell.ReadOnly = true;
            if (untenCell.Value == DBNull.Value || !(bool)untenCell.Value)
            {
                wariateCell.Value = DBNull.Value;
                wariateCell.ReadOnly = true;
            }
            else
            {
                wariateCell.ReadOnly = false;
            }
            wariateCell.UpdateBackColor(false);
            return true;
        }
      
        internal bool SetNiniToriFuka(DataGridViewCell NyuuCell, DataGridViewCell NiniCell)
        {
            if (NyuuCell == null || NiniCell == null)
                NiniCell.ReadOnly = true;
            if (NyuuCell.Value == DBNull.Value || !(bool)NyuuCell.Value)
            {
                NiniCell.Value = DBNull.Value;
                NiniCell.ReadOnly = true;
            }
            else
            {
                NiniCell.ReadOnly = false;
            }
            NiniCell.UpdateBackColor(false);
            return true;
        }
        //quoc_end
        #endregion
        ...
        /// </summary>
        //Quá trình kiểm tra số lượng dữ liệu DataGridView
        ...
        #region CreateEntityForDataGridRow(Tạo Thực Thể Cho Hàng Lưới Dữ Liệu)  
        //Tạo thực thể lưới dữ liệu
        /// <summary>
        /// データグリッドのエンティティ作成
        /// </summary>
        ...
        internal M_GENCHAKU_TIME CreateEntityForDataGridRow(DataGridViewRow row)
        {
            ...
            /*quoc_begin*/
            //MOD_WARIATE_JUN
            if (!DBNull.Value.Equals(row.Cells["MOD_WARIATE_JUN"].Value))
            {
                mGenchakuTime.MOD_WARIATE_JUN = SqlInt16.Parse(row.Cells["MOD_WARIATE_JUN"].Value.ToString());
            }

            //MOD_UNTEN_SHA
            if (!DBNull.Value.Equals(row.Cells["MOD_UNTEN_SHA"].Value))
            {
                mGenchakuTime.MOD_UNTEN_SHA = (Boolean)row.Cells["MOD_UNTEN_SHA"].Value;
            }
            else
            {
                mGenchakuTime.MOD_UNTEN_SHA = false;
            }
            //MOD_NYUURYOKU_TANTOUSHA
            if (!DBNull.Value.Equals(row.Cells["MOD_NYUURYOKU_TANTOUSHA"].Value))
            {
                mGenchakuTime.MOD_NYUURYOKU_TANTOUSHA = (Boolean)row.Cells["MOD_NYUURYOKU_TANTOUSHA"].Value;
            }
            else
            {
                mGenchakuTime.MOD_NYUURYOKU_TANTOUSHA = false;
            }

            //MOD_NINI_TORI_FUKA
            if (!DBNull.Value.Equals(row.Cells["MOD_NINI_TORI_FUKA"].Value))
            {
                mGenchakuTime.MOD_NINI_TORI_FUKA = (Boolean)row.Cells["MOD_NINI_TORI_FUKA"].Value;
            }
            else
            {
                mGenchakuTime.MOD_NINI_TORI_FUKA = false;
            }
            //MOD_RYUUBEI_KANZANKEISUU
            if (!DBNull.Value.Equals(row.Cells["MOD_RYUUBEI_KANZANKEISUU"].Value))
            {
                mGenchakuTime.MOD_RYUUBEI_KANZANKEISUU = SqlDecimal.Parse(row.Cells["MOD_RYUUBEI_KANZANKEISUU"].Value.ToString());
            }

            //// MOD_HAIKI_NAME_CD
            //if (!DBNull.Value.Equals(row.Cells["MOD_HAIKI_NAME_CD"].Value))
            //{
            //    mGenchakuTime.MOD_HAIKI_NAME_CD = (string)row.Cells["MOD_HAIKI_NAME_CD"].Value;
            //}
            // MOD_HAIKI_NAME_CD
            if (!DBNull.Value.Equals(row.Cells["MOD_HAIKI_NAME_CD"].Value))
            {
            ...
            // MOD_NISUGATA_CD
            if (!DBNull.Value.Equals(row.Cells["MOD_NISUGATA_CD"].Value))
            {
                mGenchakuTime.MOD_NISUGATA_CD = (string)row.Cells["MOD_NISUGATA_CD"].Value;
            }
            // MOD_SHOBUN_HOUHOU_CD
            if (!DBNull.Value.Equals(row.Cells["MOD_SHOBUN_HOUHOU_CD"].Value))
            {
                mGenchakuTime.MOD_SHOBUN_HOUHOU_CD = (string)row.Cells["MOD_SHOBUN_HOUHOU_CD"].Value;
            }
            //MOD_GENYO_RITSU
            if (!DBNull.Value.Equals(row.Cells["MOD_GENYO_RITSU"].Value))
            {
                mGenchakuTime.MOD_GENYO_RITSU = SqlDecimal.Parse(row.Cells["MOD_GENYO_RITSU"].Value.ToString());
            }
            //MOD_KYOBASHI
            if (!DBNull.Value.Equals(row.Cells["MOD_KYOBASHI"].Value))
            {
                mGenchakuTime.MOD_KYOBASHI = (Boolean)row.Cells["MOD_KYOBASHI"].Value;
            }
            else
            {
                mGenchakuTime.MOD_KYOBASHI = false;
            }
            /*quoc_End*/
        ...
        #region CreateEntityForDataRow(Tạo thực thể cho hàng dữ liệu)
        ...
        internal M_GENCHAKU_TIME CreateEntityForDataRow(DataRow row)
        {
            ...
            /*quoc_begin*/
            //MOD_WARIATE_JUN
            if (!DBNull.Value.Equals(row["MOD_WARIATE_JUN"]))
            {
                mGenchakuTime.MOD_WARIATE_JUN = SqlInt16.Parse(row["MOD_WARIATE_JUN"].ToString());
            }

            // MOD_UNTEN_SHA
            if (!DBNull.Value.Equals(row["MOD_UNTEN_SHA"]))
            {
                mGenchakuTime.MOD_UNTEN_SHA = (Boolean)row["MOD_UNTEN_SHA"];
            }
            else
            {
                mGenchakuTime.MOD_UNTEN_SHA = false;
            ...
            // MOD_NYUURYOKU_TANTOUSHA
            if (!DBNull.Value.Equals(row["MOD_NYUURYOKU_TANTOUSHA"]))
            {
                mGenchakuTime.MOD_NYUURYOKU_TANTOUSHA = (Boolean)row["MOD_NYUURYOKU_TANTOUSHA"];
            }
            else
            {
                mGenchakuTime.MOD_NYUURYOKU_TANTOUSHA = false;
            ...
            // MOD_NINI_TORI_FUKA
            if (!DBNull.Value.Equals(row["MOD_NINI_TORI_FUKA"]))
            {
                mGenchakuTime.MOD_NINI_TORI_FUKA = (Boolean)row["MOD_NINI_TORI_FUKA"];
            }
            else
            {
                mGenchakuTime.MOD_NINI_TORI_FUKA = false;
            ...
            //MOD_RYUUBEI_KANZANKEISUU
            if (!DBNull.Value.Equals(row["MOD_RYUUBEI_KANZANKEISUU"]))
            {
                mGenchakuTime.MOD_RYUUBEI_KANZANKEISUU = SqlDecimal.Parse(row["MOD_RYUUBEI_KANZANKEISUU"].ToString());
            }
          

            // MOD_HAIKI_NAME_CD
            if (!DBNull.Value.Equals(row["MOD_HAIKI_NAME_CD"]))
            {
                mGenchakuTime.MOD_HAIKI_NAME_CD = (string)row["MOD_HAIKI_NAME_CD"];
            }
            // MOD_NISUGATA_CD
            if (!DBNull.Value.Equals(row["MOD_NISUGATA_CD"]))
            {
                mGenchakuTime.MOD_NISUGATA_CD = (string)row["MOD_NISUGATA_CD"];
            }
            
            // MOD_SHOBUN_HOUHOU_CD
            if (!DBNull.Value.Equals(row["MOD_SHOBUN_HOUHOU_CD"]))
            {
                mGenchakuTime.MOD_SHOBUN_HOUHOU_CD = (string)row["MOD_SHOBUN_HOUHOU_CD"];
            }
            //MOD_GENYO_RITSU
            if (!DBNull.Value.Equals(row["MOD_GENYO_RITSU"]))
            {
                mGenchakuTime.MOD_GENYO_RITSU = SqlDecimal.Parse(row["MOD_GENYO_RITSU"].ToString());
            }
            //MOD_KYOBASHI
            if (!DBNull.Value.Equals(row["MOD_KYOBASHI"]))
            {
                mGenchakuTime.MOD_KYOBASHI = (Boolean)row["MOD_KYOBASHI"];
            }
            else
            {
                mGenchakuTime.MOD_KYOBASHI = false;
        ...
        public Boolean SetSearchString()
        {
            ...
            if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.Text))
            {
                ...
                if (!string.IsNullOrEmpty(this.form.CONDITION_DBFIELD.DBFieldsName))
                {
                    // 検索条件の設定
                    // 削除
                    //Cài đặt điều kiện tìm kiếm Xóa
                    if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.Text) &&
                    ...
                    //MOD_WARIATE_JUN
                    if (!string.IsNullOrEmpty(this.form.CONDITION_VALUE.Text) &&
                        this.form.CONDITION_DBFIELD.DBFieldsName.Equals("MOD_WARIATE_JUN"))
                    {
                        try
                        {
                            entity.MOD_WARIATE_JUN = SqlInt16.Parse(this.form.CONDITION_VALUE.Text);
                        }
                        catch (Exception ex)
                        {
                            MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
        ...
        /// </summary>
        /// 
        ///Nếu một hàng có cùng khóa chính tồn tại trong DB, hãy hủy kích hoạt khóa chính
        internal bool EditableToPrimaryKey()
        {
            ...
            try
            {
                ...
                /*quocbegin*/
                Func<DataGridViewCell, bool> GetBool = (cell) =>
                {
                ...
                this.form.Ichiran.Rows.Cast<DataGridViewRow>().ToList().ForEach(x => x.Cells["MOD_WARIATE_JUN"].ReadOnly = !GetBool(x.Cells["MOD_UNTEN_SHA"]));
                /*quocend*/
            }
            catch (SQLRuntimeException ex1)
            {
