    ...
    public class ShainHoshuLogic : IBuisinessLogic
    {
        ...
        public bool CreateEntity(bool isDelete)
        {
            try
            {
                ...
                //Nếu không có thay đổi so với giá trị ban đầu, hãy trả lại RowState về trạng thái ban đầu.
        ...
        public virtual void Regist(bool errorFlag)
        {
            try
            {
                ...
                //独自チェックの記述例を書く
                //Viết một ví dụ mô tả về séc của riêng bạn
                //エラーではない場合登録処理を行う
        ...
        /// <summary>
        /// 検索結果を一覧に設定
        /// Đặt kết quả tìm kiếm thành danh sách
        /// </summary>
        internal bool SetIchiran()
        {
            try
            {
                ...
                //Trong trường hợp dữ liệu tồn tại trong tài liệu định nghĩa giá trị cố định của bảng, hãy sửa đổi phần xóa, tên,
                //chữ viết tắt và thời gian áp dụng để chúng không thể thay đổi được.
        ...
        /// Trong trường hợp dữ liệu tồn tại trong tài liệu định nghĩa giá trị cố định của bảng,
        /// hãy xóa, tên, chữ viết tắt và thời gian áp dụng không thể thay đổi
        /// </summary>
        ...
        //nhận đường dẫn tệp
        private Cell GetIneiDataFilePath( int rowIndex)
        {
            string name = "MOD_INEI_DATA";
 
            return this.form.Ichiran[rowIndex, name];
        }

        public bool CheckPNGorBMP(int rowIndex)
        {
            try
            {
                LogUtility.DebugMethodStart();
                var cell = this.GetIneiDataFilePath(rowIndex);
                string x = Convert.ToString(cell.Value);
            ...
            {
                LogUtility.Error("CheckPNGorBMP", ex);
                this.form.errmessage.MessageBoxShow("E245", "");
                return true;
            }
        }

        public bool BrowseClick(int rowIndex)
        {
            try
            {
                LogUtility.DebugMethodStart(rowIndex);
                ...
                {
                    //kiểm tra xem có tồn tại đường dẫn không
                    ...
                    {
                        //dường dẫn tồn tại tiến hành mở file
