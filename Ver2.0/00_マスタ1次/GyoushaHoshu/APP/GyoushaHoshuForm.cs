    ...
    public partial class GyoushaHoshuForm : SuperForm
    {
        ...
        public virtual void Regist(object sender, EventArgs e)
        {
            if (!base.RegistErrorFlag)
            {
                ...
                if (WINDOW_TYPE.NEW_WINDOW_FLAG == this.WindowType)
                {
                    ...
                    //MessageBoxShowLogic msgLogic = new MessageBoxShowLogic();
                    //dlg = msgLogic.MessageBoxShowConfirm("諸口の現場マスタを同じ情報にて作成しますか？");
                ...
                string sGenbaCd1 = string.Empty;

                if (base.WindowType != WINDOW_TYPE.DELETE_WINDOW_FLAG)
                {
                    // ワークフローから本登録される場合、現場の登録は不要
                    //Không cần đăng ký tại chỗ nếu đăng ký chính thức được thực hiện từ quy trình làm việc.
                    if (!this.logic.IsFromShouninzumiDenshiShinseiIchiran)
                    {
                        if (null == this.logic.genbaEntity || (null != this.logic.genbaEntity && SqlBoolean.False == this.logic.genbaEntity.DELETE_FLG))
                        {
                            ...
                            if (null == this.logic.genbaEntity1)
                            {
                            ...
                            var dlg = DialogResult.None;
                            dlg = messageLogic.MessageBoxShowConfirm("諸口の現場マスタを同じ情報にて作成しますか？");
                            if (dlg == DialogResult.Yes) { 
                                catchErr = this.logic.CreateGenbaEntity1();
                                if (catchErr)
                                {
                                    return;
                                }
                                sGenbaCd1 = this.logic.genbaEntity1.GENBA_CD;
                            }
                            else
                            {
                                this.logic.genbaEntity1 = null;
                            }
                            //quoc-end
                        }
                    }
                ...
                //Di chuyển phần giao dịch vào logic.
                ...
                if (this.logic.isRegist)
                {
                    ...
                    //quoc - begin
                    if (!string.IsNullOrEmpty(sGenbaCd1))
                    {
                        switch (base.WindowType)
                        {
                            // 新規追加
                            case WINDOW_TYPE.NEW_WINDOW_FLAG:
                            case WINDOW_TYPE.UPDATE_WINDOW_FLAG:
                                this.logic.OpenGenbaForm(WINDOW_TYPE.UPDATE_WINDOW_FLAG, sGyoushaCd, sGenbaCd1);
                                break;
                            default:
                                break;
                        }
                    ...
                    else//open mode 参照 after regist(tham khảo chế độ mở sau khi đăng ký)
                    {
        ...
        private bool DupliUpdateViewCheck(EventArgs e, bool isRegister)
        {
            try
            {
                ...
                // 業者CDの入力値をゼロパディング
                //Không có đệm giá trị đầu vào CD người bán
                bool catchErr = false;
        ...
        private void EIGYOU_TANTOU_BUSHO_CD_TextChanged(object sender, EventArgs e)
        {
            ...
            // Xóa [CD Đại diện Bán hàng] [Tên Đại diện Bán hàng].
        ...
        private void MOD_URIAGE_GURUUPU_CD_Enter(object sender, EventArgs e)
        {
            this.DENPYOU_KBN_CD.Text = "1";
        }

        private void MOD_SHIHARAI_GURUUPU_CD_Enter(object sender, EventArgs e)
        {
